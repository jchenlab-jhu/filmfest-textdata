...
description.data$Datastring2 <- as.character(description.data$Datastring2)
temp <- do.call(rbind, strsplit(description.data$Datastring2, ", \"|(>\\\".*?\\\".*?\\K(, |$))", perl = TRUE))
temp2 <- do.call(rbind, strsplit(temp[,10], ",|(>\\\".*?\\\".*?\\K(, |$))", perl = TRUE))
temp3 <- cbind(temp, temp2)
temp4 <-as.data.frame(temp3)
#rename columns
colnames(temp4) <- c("phase", "counterbalance", "vidpath", "description_stop", "paired_vid", "offset", "description_content", "importance", "time", "scratch", "tag", "ID", "Trial")
temp <- temp4
...
description.data <- subset(description.data, select= -c(Datastring2))
