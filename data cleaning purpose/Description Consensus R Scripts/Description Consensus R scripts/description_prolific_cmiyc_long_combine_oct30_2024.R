...
drop.loops <- c("debugw3Dn6:debugwhDAl",
                "debugtPDp4:debugZ6HNl",
                "debugWsRzo:debuglJqup",
...
# sessionID: "debugLqFhi:debugGvqsr" in raw files
trial.data <- subset(trial.data, !(ID == 1191))
question.data <- subset(question.data, !(ID == 1191))
event.data <- subset(event.data, !(ID == 1191))
...
write.csv(demo.data, file = paste0(raw_path,"cmiyc_demo_info.csv")) #save demographics info to csv

#check for repeat participants - see Other Dropped Participants README
n_occur <- data.frame(table(demo.data$prolific_ID))
repeatIDs <- n_occur[n_occur$Freq > 1,]
...
export_sub_data_txt <- function(df, sub_ID){
  write.table(df, file = paste0(data_file_path,"cmiyclong_",sub_ID,".txt"), row.names=FALSE, col.names=FALSE, sep="\t", quote = FALSE) #save timestamp and description column
}

export_sub_data_csv <- function(df, sub_ID){
  write.csv(df, file = paste0(data_file_path,"cmiyclong_",sub_ID,".csv"), row.names=FALSE) #save timestamp and description column
}
