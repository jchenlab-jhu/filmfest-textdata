...
setwd("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data")

#create additional directories
description_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/descriptions/"
dir.create(description_path, showWarnings = FALSE)
figure_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/figures/"
dir.create(figure_path, showWarnings = FALSE)
cosmat_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/figures/cosmat/"
dir.create(cosmat_path, showWarnings = FALSE)
hist_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/figures/hist/"
dir.create(hist_path, showWarnings = FALSE)
clean_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/descriptions/clean/"
dir.create(clean_path, showWarnings = FALSE)
raw_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/descriptions/raw/"
...
trial.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/trialdata.csv", header = FALSE, comment.char="#")
#trial.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/psiturk data files/trialdata.csv", header = FALSE, comment.char="#")
colnames(trial.data) <- c("ID","Trial","Timestamp","Datastring")
#contains post-task survey data
question.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/questiondata.csv", header = FALSE, comment.char="#")
#question.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/psiturk data files/questiondata.csv", header = FALSE, comment.char="#")
colnames(question.data) <- c("ID","Question","Response")
#contains window events (e.g., focus on/off, window resize, etc)
event.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/eventdata.csv", header = FALSE, comment.char="#")
#event.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/busstop data/psiturk data files/eventdata.csv", header = FALSE, comment.char="#")
...
drop.loops <- c("debugKGgD6:debug9HTiH", #start of run 2
                "debugivleh:debugJU9xl",
                "debugNCIhG:debugXGkMI",
                "debugUNXD2:debugLq2fA",
                "debugR6JWf:debugXyZMA")
...
write.csv(demo.data, file = paste0(raw_path,"busstop_demo_info.csv")) #save demographics info to csv
...
length(loop_drop.list) #check to make sure there are no additional problematic loops
...
#parse into segments
temp <- do.call(rbind, strsplit(description.data$Datastring, ", \"|(>\\\".*?\\\".*?\\K(, |$))", perl = TRUE)) #splitting up some lines weirdly and losing columns like tag
...
temp <- as.data.frame(t(apply(as.data.frame(temp), 1, function(x) { #edit temp2 -> temp afte debug
...
temp <- temp4
#NOTE: "paired_vid" is used here in description unlike "vid_segment" from prediction because the description stops do not match 1:1 with video segments unlike prediction consensus
...
export_clean_description <- function(df,phase,counterbalance,segment){
  ...
  #NOTE: change seg to stop to avoid confusion for description; also need to change code later which is expecting to read "seg"
  write.table(df, file = paste0(clean_path,"busstop_",phase,"_c",counterbalance,"_seg",segment,".txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to csv
}
...
export_sub_data_txt <- function(df, sub_ID){
  write.table(df, file = paste0(data_file_path,"busstop_",sub_ID,".txt"), row.names=FALSE, col.names=FALSE, sep="\t", quote = FALSE) #save timestamp and description column
}

export_sub_data_csv <- function(df, sub_ID){
  write.csv(df, file = paste0(data_file_path,"busstop_",sub_ID,".csv"), row.names=FALSE) #save timestamp and description column
}
...
#CHECK IF NEXT STATEMENT WORKS (another option is to used $paired_vid instead of $description_stop)
...
use.data.n <- lapply(cosmat.list, function(x) sqrt(length(x))) #calculate number of descriptions per stop
...
#UPDATE FOR DESCRIPTION
use.data.sd <- lapply(cosmat.list, sd) #calculate number of description per stop
...
#plot average cosine similarity between USE embeddings for descriptions at each stop
...
ggsave(paste0(figure_path,"busstop_importance_timecourse.tiff"), dpi = 300, scale = 1)
...
accuracy.data <- read.csv(paste0(description_path,"busstop_stopwise_accuracy.csv"), header = TRUE, comment.char="#")
...
importance.data$experiment <- "busstop"
...
write.table(df.orig.use, file = paste0(description_path,"busstop_use_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.use, file = paste0(description_path,"busstop_use_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
df.orig.conf <- subset(importance.data, select = z_mean)
write.table(df.orig.conf, file = paste0(description_path,"busstop_conf_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.conf, file = paste0(description_path,"busstop_conf_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
df.orig.acc <- subset(accuracy.data, select = similarity)
write.table(df.orig.acc, file = paste0(description_path,"busstop_acc_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.acc, file = paste0(description_path,"busstop_acc_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv

write.csv(tr, file = paste0(description_path,"busstop_full_interp_tr_match.csv")) #save full dataframe to csv
...
#write.table(df.use, file = paste0(description_path,"busstop_use_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.use, file = paste0(description_path,"busstop_use_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.use, file = paste0(description_path,"busstop_use_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.use, file = paste0(description_path,"busstop_use_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
#write.table(df.conf, file = paste0(description_path,"busstop_conf_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.conf, file = paste0(description_path,"busstop_conf_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.conf, file = paste0(description_path,"busstop_conf_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.conf, file = paste0(description_path,"busstop_conf_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
#write.table(df.acc, file = paste0(description_path,"busstop_acc_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.acc, file = paste0(description_path,"busstop_acc_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.acc, file = paste0(description_path,"busstop_acc_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.acc, file = paste0(description_path,"busstop_acc_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
ggplot(data=tr, aes(x=s, y=importance)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("importance (z)") + ylim(-1, 1)
ggsave(paste0(figure_path,"busstop_importance_upsampled_timecourse.png"), dpi = 300, scale = 1)
