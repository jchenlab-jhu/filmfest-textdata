...
setwd("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theboyfriend data")

#create additional directories
description_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theboyfriend data/descriptions/"
dir.create(description_path, showWarnings = FALSE)
figure_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theboyfriend data/figures/"
dir.create(figure_path, showWarnings = FALSE)
cosmat_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theboyfriend data/figures/cosmat/"
dir.create(cosmat_path, showWarnings = FALSE)
hist_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theboyfriend data/figures/hist/"
dir.create(hist_path, showWarnings = FALSE)
clean_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theboyfriend data/descriptions/clean/"
dir.create(clean_path, showWarnings = FALSE)
raw_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theboyfriend data/descriptions/raw/"
dir.create(raw_path, showWarnings = FALSE)

##load data
#contains trial data & keywords
trial.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theboyfriend data/trialdata.csv", header = FALSE, comment.char="#")
colnames(trial.data) <- c("ID","Trial","Timestamp","Datastring")
#contains post-task survey data
question.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theboyfriend data/questiondata.csv", header = FALSE, comment.char="#")
colnames(question.data) <- c("ID","Question","Response")
#contains window events (e.g., focus on/off, window resize, etc)
event.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theboyfriend data/eventdata.csv", header = FALSE, comment.char="#")
...
drop.loops <- c("debugqYWsZ:debugMovMp",
                "debugIKZtO:debugLTqgk",
                "debugC6OiV:debugFVNPi",
                "debugysYGR:debugzZAsj", 
                "debugyf7Ui:debug7sCDT")
...
# drop for separate issue - see Other Dropped Participants README
# sessionID: "debug0B9m8:debug3FUu6" in raw files
trial.data <- subset(trial.data, !(ID == 1088))
question.data <- subset(question.data, !(ID == 1088))
event.data <- subset(event.data, !(ID == 1088))
...
write.csv(demo.data, file = paste0(raw_path,"theboyfriend_demo_info.csv")) #save demographics info to csv
...
#previous: colnames(temp4) <- c("phase", "counterbalance", "description_stop", "importance", "offset", "vidpath", "paired_vid", "description_content", "time", "scratch", "tag", "ID", "Trial")
...
export_clean_description <- function(df,phase,counterbalance,segment){
  ...
  write.table(df, file = paste0(clean_path,"theboyfriend_",phase,"_c",counterbalance,"_seg",segment,".txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to csv
}
...
export_sub_data_txt <- function(df, sub_ID){
  write.table(df, file = paste0(data_file_path,"theboyfriend_",sub_ID,".txt"), row.names=FALSE, col.names=FALSE, sep="\t", quote = FALSE) #save timestamp and description column
}

export_sub_data_csv <- function(df, sub_ID){
  write.csv(df, file = paste0(data_file_path,"theboyfriend_",sub_ID,".csv"), row.names=FALSE) #save timestamp and description column
}
...
ggsave(paste0(figure_path,"theboyfriend_importance_timecourse.tiff"), dpi = 300, scale = 1)
...
accuracy.data <- read.csv(paste0(description_path,"theboyfriend_stopwise_accuracy.csv"), header = TRUE, comment.char="#")
...
importance.data$experiment <- "theboyfriend"
...
write.table(df.orig.use, file = paste0(description_path,"theboyfriend_use_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.use, file = paste0(description_path,"theboyfriend_use_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
df.orig.conf <- subset(importance.data, select = z_mean)
write.table(df.orig.conf, file = paste0(description_path,"theboyfriend_conf_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.conf, file = paste0(description_path,"theboyfriend_conf_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
df.orig.acc <- subset(accuracy.data, select = similarity)
write.table(df.orig.acc, file = paste0(description_path,"theboyfriend_acc_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.acc, file = paste0(description_path,"theboyfriend_acc_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv

write.csv(tr, file = paste0(description_path,"theboyfriend_full_interp_tr_match.csv")) #save full dataframe to csv
...
#write.table(df.use, file = paste0(description_path,"theboyfriend_use_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.use, file = paste0(description_path,"theboyfriend_use_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.use, file = paste0(description_path,"theboyfriend_use_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.use, file = paste0(description_path,"theboyfriend_use_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
#write.table(df.conf, file = paste0(description_path,"theboyfriend_conf_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.conf, file = paste0(description_path,"theboyfriend_conf_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.conf, file = paste0(description_path,"theboyfriend_conf_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.conf, file = paste0(description_path,"theboyfriend_conf_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
#write.table(df.acc, file = paste0(description_path,"theboyfriend_acc_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.acc, file = paste0(description_path,"theboyfriend_acc_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.acc, file = paste0(description_path,"theboyfriend_acc_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.acc, file = paste0(description_path,"theboyfriend_acc_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
ggplot(data=tr, aes(x=s, y=importance)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("importance (z)") + ylim(-1, 1)
ggsave(paste0(figure_path,"theboyfriend_importance_upsampled_timecourse.png"), dpi = 300, scale = 1)
...
#define movie of interest
#potential labels: "cmiyc_long", "theboyfriend", "theboyfriend", "theboyfriend", "therock", "theshoe"
movie <- "theboyfriend"
