...
setwd("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theshoe data")

#create additional directories
description_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theshoe data/descriptions/"
dir.create(description_path, showWarnings = FALSE)
figure_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theshoe data/figures/"
dir.create(figure_path, showWarnings = FALSE)
cosmat_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theshoe data/figures/cosmat/"
dir.create(cosmat_path, showWarnings = FALSE)
hist_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theshoe data/figures/hist/"
dir.create(hist_path, showWarnings = FALSE)
clean_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theshoe data/descriptions/clean/"
dir.create(clean_path, showWarnings = FALSE)
raw_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theshoe data/descriptions/raw/"
dir.create(raw_path, showWarnings = FALSE)

##load data
#contains trial data & keywords
trial.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theshoe data/trialdata.csv", header = FALSE, comment.char="#")
colnames(trial.data) <- c("ID","Trial","Timestamp","Datastring")
#contains post-task survey data
question.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theshoe data/questiondata.csv", header = FALSE, comment.char="#")
colnames(question.data) <- c("ID","Question","Response")
#contains window events (e.g., focus on/off, window resize, etc)
event.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/theshoe data/eventdata.csv", header = FALSE, comment.char="#")
...
drop.loops <- c("debugaG7tQ:debugfWmQi",
                "debugl8I4G:debugLk1Sw",
                "debugh282v:debugLSJas",
                "debugFVMVB:debugXdtFZ") # drop for separate issue - see Other Dropped Participants README
...
#editing out problematic parsing (datastrings that end with a , )
trial.data$Datastring <- gsub(', ", "importance','", "importance',trial.data$Datastring)

#update ID column to experimental IDs (to anonymize original ID)
trial.data$sessionID <- trial.data$ID #create new sessionID column
trial.data$ID <- 0 #replace old ID column with 0s
#replace sessionIDs with experimental IDs
j <- 1000 #IDs will begin at 1000
for(i in unique(trial.data$sessionID)){ #for all unique IDs
  trial.data$ID[trial.data$sessionID==i] <- j #set ID to j
  j <- j + 1 #increment j
}
...
#event.data
event.data$sessionID <- event.data$ID #create new sessionID column
event.data$ID <- 0 #replace old ID column with 0s
event.data$ID <- with(trial.data, ID[match(event.data$sessionID, sessionID)])
#question.data
question.data$sessionID <- question.data$ID #create new sessionID column
question.data$ID <- 0 #replace old ID column with 0s
question.data$ID <- with(trial.data, ID[match(question.data$sessionID, sessionID)])

#delete unnecessary columns
# trial.data <- subset(trial.data, select= -c(sessionID))
# event.data <- subset(event.data, select= -c(sessionID))
# question.data <- subset(question.data, select= -c(sessionID))
...
#extract and match demographic information of retained participants
demo.data<- as.data.frame(unique(trial.data$ID))
colnames(demo.data) <- c("internal_ID")
demo.data$session_ID <- trial.data$sessionID[match(demo.data$internal_ID, trial.data$ID)]
#extract prolific IDs
demo_temp <- subset(question.data, question.data$Question == "prolific_ID2")
demo.data$prolific_ID <- demo_temp$Response[match(demo.data$session_ID, demo_temp$sessionID)]
#extract demo info
demo_temp <- subset(question.data, question.data$Question == "demographics_age")
demo.data$age <- demo_temp$Response[match(demo.data$session_ID, demo_temp$sessionID)]
demo_temp <- subset(question.data, question.data$Question == "demographics_gender")
demo.data$gender <- demo_temp$Response[match(demo.data$session_ID, demo_temp$sessionID)]
demo_temp <- subset(question.data, question.data$Question == "demographics_hand")
demo.data$dominant_hand <- demo_temp$Response[match(demo.data$session_ID, demo_temp$sessionID)]
demo_temp <- subset(question.data, question.data$Question == "demographics_nativelang")
demo.data$english_nativelang <- demo_temp$Response[match(demo.data$session_ID, demo_temp$sessionID)]
demo_temp <- subset(question.data, question.data$Question == "demographics_nativelang_text")
demo.data$nativelang_text <- demo_temp$Response[match(demo.data$session_ID, demo_temp$sessionID)]
demo_temp <- subset(question.data, question.data$Question == "demographics_fluency")
demo.data$english_fluent <- demo_temp$Response[match(demo.data$session_ID, demo_temp$sessionID)]
demo_temp <- subset(question.data, question.data$Question == "demographics_fluency_text")
demo.data$fluentlang_text <- demo_temp$Response[match(demo.data$session_ID, demo_temp$sessionID)]
demo_temp <- subset(question.data, question.data$Question == "demographics_race")
demo.data$race <- demo_temp$Response[match(demo.data$session_ID, demo_temp$sessionID)]
demo.data$race <- replace(demo.data$race, which(demo.data$race == "MORE"), "Multiracial")
demo.data$race <- replace(demo.data$race, which(demo.data$race == "NONE"), "Prefer not to say")
demo_temp <- subset(question.data, question.data$Question == "demographics_hispanic")
demo.data$hispanic <- demo_temp$Response[match(demo.data$session_ID, demo_temp$sessionID)]
demo_temp <- subset(question.data, question.data$Question == "demographics_education")
demo.data$education_level <- demo_temp$Response[match(demo.data$session_ID, demo_temp$sessionID)]
demo.data$education_level <- replace(demo.data$education_level, which(demo.data$education_level == 0), "Less than high school degree")
demo.data$education_level <- replace(demo.data$education_level, which(demo.data$education_level == 1), "High school degree or equivalent")
demo.data$education_level <- replace(demo.data$education_level, which(demo.data$education_level == 2), "Some college but no degree")
demo.data$education_level <- replace(demo.data$education_level, which(demo.data$education_level == 3), "Associate degree")
demo.data$education_level <- replace(demo.data$education_level, which(demo.data$education_level == 4), "Bachelor degree")
demo.data$education_level <- replace(demo.data$education_level, which(demo.data$education_level == 5), "Masters degree")
demo.data$education_level <- replace(demo.data$education_level, which(demo.data$education_level == 6), "Doctoral degree")
write.csv(demo.data, file = paste0(raw_path,"theshoe_demo_info.csv")) #save demographics info to csv
...
# spoiler.data$dataset <- "theshoe"
# #drop sessionID from question.data
# question.data <- subset(question.data, select= -c(sessionID))
# #drop ID from spoiler.data
# spoiler.data <- subset(spoiler.data, select= -c(ID))
# #total number of subjects
# length(unique(spoiler.data$sessionID))
# #export spoiler questions as csv
# write.csv(spoiler.data, file = paste0(raw_path,"theshoe_spoilerdata.csv")) #save dataframe to csv

#######################
###experimental data###
#######################

#for theshoe only: the offset times were erroneously listed in the experiment code
#replacing the erroneous offset times with the correct ones here:

trial.data$Datastring <- ifelse(grepl("offset\": \"140",trial.data$Datastring,fixed = TRUE) & grepl("/static/video/B/filmfest",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"140", "offset\": \"135", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)
trial.data$Datastring <- ifelse(grepl("offset\": \"150",trial.data$Datastring,fixed = TRUE) & grepl("/static/video/C/filmfest",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"150", "offset\": \"135", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)
trial.data$Datastring <- ifelse(grepl("offset\": \"160",trial.data$Datastring,fixed = TRUE) & grepl("/static/video/D/filmfest",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"160", "offset\": \"135", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)

trial.data$Datastring <- ifelse(grepl("offset\": \"140",trial.data$Datastring,fixed = TRUE) & grepl("/static/video/E/filmfest",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"140", "offset\": \"145", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)
trial.data$Datastring <- ifelse(grepl("offset\": \"150",trial.data$Datastring,fixed = TRUE) & grepl("/static/video/F/filmfest",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"150", "offset\": \"145", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)
trial.data$Datastring <- ifelse(grepl("offset\": \"160",trial.data$Datastring,fixed = TRUE) & grepl("/static/video/G/filmfest",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"160", "offset\": \"145", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)

trial.data$Datastring <- ifelse(grepl("offset\": \"170",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"170", "offset\": \"145", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)
trial.data$Datastring <- ifelse(grepl("offset\": \"180",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"180", "offset\": \"145", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)
trial.data$Datastring <- ifelse(grepl("offset\": \"190",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"190", "offset\": \"145", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)
trial.data$Datastring <- ifelse(grepl("offset\": \"200",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"200", "offset\": \"145", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)
trial.data$Datastring <- ifelse(grepl("offset\": \"210",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"210", "offset\": \"145", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)
trial.data$Datastring <- ifelse(grepl("offset\": \"220",trial.data$Datastring,fixed = TRUE), gsub("offset\": \"220", "offset\": \"145", trial.data$Datastring, fixed = TRUE), trial.data$Datastring)
...
export_clean_description <- function(df,phase,counterbalance,segment){
  ...
  write.table(df, file = paste0(clean_path,"theshoe_",phase,"_c",counterbalance,"_seg",segment,".txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to csv
}
...
#subject data files
data_file_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/subject_data_files/"
dir.create(data_file_path, showWarnings = FALSE)

sub_data <- select(description.data, ID, Timestamp, description_content, phase, description_stop)
sub_data_files_df <- subset(subset(sub_data, phase == "test"), select = -c(phase, description_stop))
sub_data_IDs <- as.vector(levels(unique(sub_data_files_df$ID)))

export_sub_data_txt <- function(df, sub_ID){
  write.table(df, file = paste0(data_file_path,"theshoe_",sub_ID,".txt"), row.names=FALSE, col.names=FALSE, sep="\t", quote = FALSE) #save timestamp and description column
}

export_sub_data_csv <- function(df, sub_ID){
  write.csv(df, file = paste0(data_file_path,"theshoe_",sub_ID,".csv"), row.names=FALSE) #save timestamp and description column
}

for(i in 1:length(sub_data_IDs)) {
  data_file_df <- subset(sub_data_files_df, ID == sub_data_IDs[i])
  data_file_df <- subset(data_file_df, select = -c(ID))
  export_sub_data_txt(data_file_df, sub_data_IDs[i])
  export_sub_data_csv(data_file_df, sub_data_IDs[i])
}
...
ggsave(paste0(figure_path,"theshoe_importance_timecourse.tiff"), dpi = 300, scale = 1)
...
accuracy.data <- read.csv(paste0(description_path,"theshoe_stopwise_accuracy.csv"), header = TRUE, comment.char="#")
...
ggsave(paste0(figure_path,"theshoe_accuracy_timecourse.png"), dpi = 300, scale = 1)
...
importance.data$experiment <- "theshoe"
use.data$experiment <- "theshoe"
accuracy.data$experiment <- "theshoe"
...
ggsave(paste0(figure_path,"theshoe_use_conf_scatter.png"), scale = 1)
...
ggplot(importance.data,aes(x=z_mean, y=accuracy.data$similarity))+
  ...
  scale_x_continuous(limits = c(-.8,.8)) +
  stat_cor(method = "pearson", label.x = (median((-8:8)/10)), label.y = .675) #from ggpubr -- plots r and p-value
ggsave(paste0(figure_path,"theshoe_accuracy_conf_scatter.png"), scale = 1)
...
ggsave(paste0(figure_path,"theshoe_accuracy_use_scatter.png"), scale = 1)
...
write.table(df.orig.use, file = paste0(description_path,"theshoe_use_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.use, file = paste0(description_path,"theshoe_use_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
df.orig.conf <- subset(importance.data, select = z_mean)
write.table(df.orig.conf, file = paste0(description_path,"theshoe_conf_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.conf, file = paste0(description_path,"theshoe_conf_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
df.orig.acc <- subset(accuracy.data, select = similarity)
write.table(df.orig.acc, file = paste0(description_path,"theshoe_acc_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.acc, file = paste0(description_path,"theshoe_acc_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv

write.csv(tr, file = paste0(description_path,"theshoe_full_interp_tr_match.csv")) #save full dataframe to csv
...
#write.table(df.use, file = paste0(description_path,"theshoe_use_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.use, file = paste0(description_path,"theshoe_use_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.use, file = paste0(description_path,"theshoe_use_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.use, file = paste0(description_path,"theshoe_use_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
#write.table(df.conf, file = paste0(description_path,"theshoe_conf_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.conf, file = paste0(description_path,"theshoe_conf_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.conf, file = paste0(description_path,"theshoe_conf_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.conf, file = paste0(description_path,"theshoe_conf_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
#write.table(df.acc, file = paste0(description_path,"theshoe_acc_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.acc, file = paste0(description_path,"theshoe_acc_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.acc, file = paste0(description_path,"theshoe_acc_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.acc, file = paste0(description_path,"theshoe_acc_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
ggplot(data=tr, aes(x=s, y=similarity)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("cosine similarity") + ylim(.2, .7)
ggsave(paste0(figure_path,"theshoe_use_upsampled_timecourse.png"), dpi = 300, scale = 1)
...
ggplot(data=tr, aes(x=s, y=importance)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("importance (z)") + ylim(-1, 1)
ggsave(paste0(figure_path,"theshoe_importance_upsampled_timecourse.png"), dpi = 300, scale = 1)
...
ggplot(data=tr, aes(x=s, y=accuracy)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("accuracy") + ylim(.2, .7)
ggsave(paste0(figure_path,"theshoe_accuracy_upsampled_timecourse.png"), dpi = 300, scale = 1)
...
setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/theshoe/roi_tc/tc")
temp = list.files(pattern="*.txt")
tc_list = lapply(temp, function(x) read.delim(x, header = FALSE, sep = ","))
#load group averaged pISC ROI timecourses
setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/theshoe/roi_tc/pisc")
...
for (i in 1:length(tc_list)){
  ...
  #set path
  setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/theshoe/roi_tc/")
  ...
  ggsave(paste0("theshoe_",stri_sub(stri_sub(temp[i],12),1,-16),"_timecourses.png"), scale = 2)
...
#define movie of interest
#potential labels: "cmiyc_long", "theshoe", "theshoe", "theshoe", "theshoe", "theshoe"
movie <- "theshoe"
