...
setwd("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/cmiyc_long_1")

#create additional directories
description_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/cmiyc_long_1/descriptions/"
dir.create(description_path, showWarnings = FALSE)
figure_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/cmiyc_long_1/figures/"
dir.create(figure_path, showWarnings = FALSE)
cosmat_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/cmiyc_long_1/figures/cosmat/"
dir.create(cosmat_path, showWarnings = FALSE)
hist_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/cmiyc_long_1/figures/hist/"
dir.create(hist_path, showWarnings = FALSE)
clean_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/cmiyc_long_1/descriptions/clean/"
dir.create(clean_path, showWarnings = FALSE)
raw_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/cmiyc_long_1/descriptions/raw/"
dir.create(raw_path, showWarnings = FALSE)

##load data
#contains trial data & keywords
trial.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/cmiyc_long_1/trialdata.csv", header = FALSE, comment.char="#")
colnames(trial.data) <- c("ID","Trial","Timestamp","Datastring")
#contains post-task survey data
question.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/cmiyc_long_1/questiondata.csv", header = FALSE, comment.char="#")
colnames(question.data) <- c("ID","Question","Response")
#contains window events (e.g., focus on/off, window resize, etc)
event.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/cmiyc_long_1/eventdata.csv", header = FALSE, comment.char="#")
...
# #NOTE: for cmiyc_long_1, don't run this section for now - TODO: COME BACK + uncomment
...
export_clean_description <- function(df,phase,counterbalance,segment){
  ...
  write.table(df, file = paste0(clean_path,"cmiyc_long_1_",phase,"_c",counterbalance,"_seg",segment,".txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to csv
}
#define function to save all descriptions in raw format
export_raw_description <- function(df, phase){
  write.csv(df, file = paste0(raw_path,"cmiyc_long_1_",phase,"_full.csv")) #save full dataframe to csv
}
