...
setwd("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/therock data")

#create additional directories
description_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/therock data/descriptions/"
dir.create(description_path, showWarnings = FALSE)
figure_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/therock data/figures/"
dir.create(figure_path, showWarnings = FALSE)
cosmat_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/therock data/figures/cosmat/"
dir.create(cosmat_path, showWarnings = FALSE)
hist_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/therock data/figures/hist/"
dir.create(hist_path, showWarnings = FALSE)
clean_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/therock data/descriptions/clean/"
dir.create(clean_path, showWarnings = FALSE)
raw_path <- "D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/therock data/descriptions/raw/"
dir.create(raw_path, showWarnings = FALSE)

##load data
#contains trial data & keywords
trial.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/therock data/trialdata.csv", header = FALSE, comment.char="#")
colnames(trial.data) <- c("ID","Trial","Timestamp","Datastring")
#contains post-task survey data
question.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/therock data/questiondata.csv", header = FALSE, comment.char="#")
colnames(question.data) <- c("ID","Question","Response")
#contains window events (e.g., focus on/off, window resize, etc)
event.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Description Consensus/therock data/eventdata.csv", header = FALSE, comment.char="#")
...
drop.loops <- c("debugju8sj:debugjXOgS",
                "debuga9k3G:debugDSmuE",
                "debugXiOXc:debuggnDvN",
                "debugmk0pY:debugZBlwR",
                "debug3ZAq4:debugWtBYs",
                "debug8LVUl:debugTfWtU",
                "debugArDN3:debugPsdC1",
                "debugeNJ2L:debugmY0uI",
                "debugDefZV:debugvAJ5G",
                "debugaDyAS:debugrackF")
...
comprehension <- within(comprehension, {
  question_text = ifelse(comprehension$Question=="Comp_Q1", "What was the title of this clip, presented in a white font on a black background?", ifelse(comprehension$Question== "Comp_Q2","Over how much time did this story take place?", ifelse(comprehension$Question== "Comp_Q3", "Which of the following did not happen to the protagonist?", ifelse(comprehension$Question== "Comp_Q4", "What did the rock do at the end of the movie?", "ERROR"))))
})
...
catch.drop.data <- subset(trial.data, ID %in% catch.drop.list) %>% distinct(ID, counterbalance, sessionID) #for debugging purposes
...
comp.drop.data <- subset(trial.data, ID %in% comp.drop.list) %>% distinct(ID, counterbalance, sessionID) #for debugging purposes
comp.drop.Qdata <- subset(question.data, ID %in% comp.drop.list) #for debugging purposes
...
write.csv(demo.data, file = paste0(raw_path,"therock_demo_info.csv")) #save demographics info to csv
...
#TODO: need to debug this section
# #subset spoiler data
...
# write.csv(spoiler.data, file = paste0(raw_path,"therock_spoilerdata.csv")) #save dataframe to csv
...
export_clean_description <- function(df,phase,counterbalance,segment){
  ...
  write.table(df, file = paste0(clean_path,"therock_",phase,"_c",counterbalance,"_seg",segment,".txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to csv
}
#define function to save all descriptions in raw format
export_raw_description <- function(df, phase){
  write.csv(df, file = paste0(raw_path,"therock_",phase,"_full.csv")) #save full dataframe to csv
}
...
export_sub_data_txt <- function(df, sub_ID){
  write.table(df, file = paste0(data_file_path,"therock_",sub_ID,".txt"), row.names=FALSE, col.names=FALSE, sep="\t", quote = FALSE) #save timestamp and description column
}

export_sub_data_csv <- function(df, sub_ID){
  write.csv(df, file = paste0(data_file_path,"therock_",sub_ID,".csv"), row.names=FALSE) #save timestamp and description column
}
...
ggsave(paste0(figure_path,"therock_importance_timecourse.tiff"), dpi = 300, scale = 1)
...
accuracy.data <- read.csv(paste0(description_path,"therock_stopwise_accuracy.csv"), header = TRUE, comment.char="#")
...
importance.data$experiment <- "therock"
...
ggsave(paste0(figure_path,"therock_use_conf_scatter.png"), scale = 1)
...
write.table(df.orig.use, file = paste0(description_path,"therock_use_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.use, file = paste0(description_path,"therock_use_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
df.orig.conf <- subset(importance.data, select = z_mean)
write.table(df.orig.conf, file = paste0(description_path,"therock_conf_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.conf, file = paste0(description_path,"therock_conf_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
df.orig.acc <- subset(accuracy.data, select = similarity)
write.table(df.orig.acc, file = paste0(description_path,"therock_acc_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.orig.acc, file = paste0(description_path,"therock_acc_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv

write.csv(tr, file = paste0(description_path,"therock_full_interp_tr_match.csv")) #save full dataframe to csv
...
#write.table(df.use, file = paste0(description_path,"therock_use_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.use, file = paste0(description_path,"therock_use_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.use, file = paste0(description_path,"therock_use_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.use, file = paste0(description_path,"therock_use_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
#write.table(df.conf, file = paste0(description_path,"therock_conf_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.conf, file = paste0(description_path,"therock_conf_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.conf, file = paste0(description_path,"therock_conf_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.conf, file = paste0(description_path,"therock_conf_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
#write.table(df.acc, file = paste0(description_path,"therock_acc_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
#write.table(df.acc, file = paste0(description_path,"therock_acc_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
write.table(df.acc, file = paste0(description_path,"therock_acc_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save description column only, without row or column labels, to csv
write.table(df.acc, file = paste0(description_path,"therock_acc_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save description column only, without row or column labels, to txt
...
ggplot(data=tr, aes(x=s, y=similarity)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("cosine similarity") + ylim(.2, .7)
ggsave(paste0(figure_path,"therock_use_upsampled_timecourse.png"), dpi = 300, scale = 1)
...
ggplot(data=tr, aes(x=s, y=importance)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("importance (z)") + ylim(-1, 1)
ggsave(paste0(figure_path,"therock_importance_upsampled_timecourse.png"), dpi = 300, scale = 1)
...
#define movie of interest
#potential labels: "cmiyc_long", "therock", "therock", "therock", "therock", "theshoe"
movie <- "therock"
