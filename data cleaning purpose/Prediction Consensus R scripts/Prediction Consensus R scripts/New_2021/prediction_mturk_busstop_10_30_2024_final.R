...
setwd("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/busstop")

#create additional directories
prediction_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/busstop/final/predictions/"
dir.create(prediction_path, showWarnings = FALSE)
figure_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/busstop/final/figures/"
dir.create(figure_path, showWarnings = FALSE)
cosmat_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/busstop/final/figures/cosmat/"
dir.create(cosmat_path, showWarnings = FALSE)
hist_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/busstop/final/figures/hist/"
dir.create(hist_path, showWarnings = FALSE)
clean_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/busstop/final/predictions/clean/"
dir.create(clean_path, showWarnings = FALSE)
raw_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/busstop/final/predictions/raw/"
dir.create(raw_path, showWarnings = FALSE)

##load data
#contains trial data & keywords
trial.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/busstop/trialdata.csv", header = FALSE, comment.char="#")
colnames(trial.data) <- c("ID","Trial","Timestamp","Datastring")
#contains post-task survey data
question.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/busstop/questiondata.csv", header = FALSE, comment.char="#")
colnames(question.data) <- c("ID","Question","Response")
#contains window events (e.g., focus on/off, window resize, etc)
event.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/busstop/eventdata.csv", header = FALSE, comment.char="#")
...
drop.loops <- c("debugs2Uin:debugUss8N",
...
write.csv(spoiler.data, file = paste0(raw_path,"busstop_spoilerdata.csv")) #save dataframe to csv
...
##NOTE: this is causing a problem because some row are out of order
...
#previously: prediction.data$Datastring2 <- as.character(prediction.data$Datastring2)
...
#old string: c("phase", "counterbalance", "vidpath", "video_segment", "offset", "prediction_number", "prediction_content", "confidence", "time", "scratch", "tag", "ID", "Trial")
...
#drop 31-9 participants in counterbalance 3 (accidentally collected) in order to balance out distribution
prediction_final.data <- prediction.data
cb3.data <- subset(prediction_final.data, counterbalance == 3) #data frame of only counterbalance 3
cb3_ID.data <- distinct(cb3.data, ID) #list of unique IDs of all people collected for cb3 
cb3_removed.data <- tail(cb3_ID.data, nrow(cb3_ID.data)-9)
cb3_ID.data <- head(cb3_ID.data, 9) #list of final cb3 IDs kept
cb3_removed_indices <- unlist(lapply(cb3_removed.data$ID, function(x) which(prediction_final.data$ID %in% x)))
cb3_removed.data <- prediction_final.data %>%  filter(row_number() %in% cb3_removed_indices)
prediction_final.data <- prediction_final.data %>%  filter(!row_number() %in% cb3_removed_indices)

# use tail() to extract the rows that need to be taken out
# then use match() to find index of those rows
# then remove those specific indices in that data frame
...
export_clean_prediction <- function(df,phase,counterbalance,segment){
  ...
  write.table(df, file = paste0(clean_path,"busstop_",phase,"_c",counterbalance,"_seg",segment,".txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to csv
  write.csv(df, file = paste0(raw_path,"busstop_",phase,"_prediction_data_clean.csv")) #save full dataframe to csv 
}
#define function to save all predictions in raw format
export_raw_prediction <- function(df, phase){
  write.csv(df, file = paste0(raw_path,"busstop_",phase,"_full.csv")) #save full dataframe to csv
}

#define function to save data frame as R variable to use for merged data
export_data_frame <- function(df) {
  setwd("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/busstop/final")
  df2 <- df %>%
...
practice_cAllsAll <- subset(prediction_final.data, phase == "practice")
export_raw_prediction(practice_cAllsAll, "practice")
practice_cAllsA <- subset(prediction_final.data, video_segment == "A", select = c(prediction_content))
export_clean_prediction(practice_cAllsA, "practice", "All", "A")
practice_cAllsB <- subset(prediction_final.data, video_segment == "B", select = c(prediction_content))
export_clean_prediction(practice_cAllsB, "practice", "All", "B")
##phase: test
##predictions: all
test_cAllsAll <- subset(prediction_final.data, phase == "test")
export_raw_prediction(test_cAllsAll, "test")
export_data_frame(prediction.data)
#counterbalance: all (for title only)
test_cAllsegTitle <- prediction_final.data[grep("filmfest_clip12_title_7s.mp4", prediction_final.data$vidpath), ]
...
#counterbalance: 3
test_c3s1 <- subset(prediction_final.data, counterbalance == 3 & video_segment == "1", select = c(prediction_content))
export_clean_prediction(test_c3s1, "test", "3", "1")
test_c3s2 <- subset(prediction_final.data, counterbalance == 3 & video_segment == "2", select = c(prediction_content))
export_clean_prediction(test_c3s2, "test", "3", "2")
test_c3s3 <- subset(prediction_final.data, counterbalance == 3 & video_segment == "3", select = c(prediction_content))
export_clean_prediction(test_c3s3, "test", "3", "3")
test_c3s4 <- subset(prediction_final.data, counterbalance == 3 & video_segment == "4", select = c(prediction_content))
export_clean_prediction(test_c3s4, "test", "3", "4")
test_c3s5 <- subset(prediction_final.data, counterbalance == 3 & video_segment == "5", select = c(prediction_content))
export_clean_prediction(test_c3s5, "test", "3", "5")
test_c3s6 <- subset(prediction_final.data, counterbalance == 3 & video_segment == "6", select = c(prediction_content))
export_clean_prediction(test_c3s6, "test", "3", "6")
test_c3s7 <- subset(prediction_final.data, counterbalance == 3 & video_segment == "7", select = c(prediction_content))
export_clean_prediction(test_c3s7, "test", "3", "7")
test_c3s8 <- subset(prediction_final.data, counterbalance == 3 & video_segment == "8", select = c(prediction_content))
...
#path to python script: /Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments//Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/busstop/predictions
...
ggsave(paste0(figure_path,"busstop_use_cosmat.pdf"), scale = 2)
...
ggsave(paste0(figure_path,"busstop_use_histogram.pdf"), scale = 2)
...
ggsave(paste0(figure_path,"busstop_use_timecourse.tiff"), dpi = 300, scale = 1)
ggsave(paste0(figure_path,"busstop_use_timecourse.png"), dpi = 300, scale = 1)
...
ggplot(data=use.data, aes(x=offset, y=similarity)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("cosine similarity") + ylim(-0.1, 0.8)
ggsave(paste0(figure_path,"busstop_use_timecourse_labelled.png"), dpi = 300, scale = 1)
...
confidence.data$counterbalance[confidence.data$offset == 7] <- NA
...
ggsave(paste0(figure_path,"busstop_confidence_timecourse.tiff"), dpi = 300, scale = 1)
ggsave(paste0(figure_path,"busstop_confidence_timecourse.png"), dpi = 300, scale = 1)
...
ggplot(data=confidence.data, aes(x=offset, y=z_mean)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("confidence (z)") + ylim(-1.1, 1.5)
ggsave(paste0(figure_path,"busstop_confidence_timecourse_labelled.png"), dpi = 300, scale = 1)
...
accuracy.data <- read.csv(paste0(prediction_path,"busstop_stopwise_accuracy.csv"), header = TRUE, comment.char="#")
...
ggsave(paste0(figure_path,"busstop_accuracy_timecourse.png"), dpi = 300, scale = 1)
...
confidence.data$experiment <- "busstop"
use.data$experiment <- "busstop"
accuracy.data$experiment <- "busstop"
...
ggsave(paste0(figure_path,"busstop_use_conf_scatter.png"), scale = 1)
...
ggplot(confidence.data,aes(x=z_mean, y=accuracy.data$similarity))+
  ...
  scale_x_continuous(limits = c(-.8,.8)) +
  stat_cor(method = "pearson", label.x = (median((-8:8)/10)), label.y = .675) #from ggpubr -- plots r and p-value
ggsave(paste0(figure_path,"busstop_accuracy_conf_scatter.png"), scale = 1)
...
ggsave(paste0(figure_path,"busstop_accuracy_use_scatter.png"), scale = 1)



##upsample USE/confidence/accuracy timecourses to fMRI timecourse
#spline for use
...
#build a vector representing each TR, in seconds, for the busstop movie
...
write.table(df.orig.use, file = paste0(prediction_path,"busstop_use_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.use, file = paste0(prediction_path,"busstop_use_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
df.orig.conf <- subset(confidence.data, select = z_mean)
write.table(df.orig.conf, file = paste0(prediction_path,"busstop_conf_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.conf, file = paste0(prediction_path,"busstop_conf_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
df.orig.acc <- subset(accuracy.data, select = similarity)
write.table(df.orig.acc, file = paste0(prediction_path,"busstop_acc_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.acc, file = paste0(prediction_path,"busstop_acc_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv

write.csv(tr, file = paste0(prediction_path,"busstop_full_interp_tr_match.csv")) #save full dataframe to csv
...
#write.table(df.use, file = paste0(prediction_path,"busstop_use_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#write.table(df.use, file = paste0(prediction_path,"busstop_use_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
write.table(df.use, file = paste0(prediction_path,"busstop_use_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.use, file = paste0(prediction_path,"busstop_use_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
...
#write.table(df.conf, file = paste0(prediction_path,"busstop_conf_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#write.table(df.conf, file = paste0(prediction_path,"busstop_conf_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
write.table(df.conf, file = paste0(prediction_path,"busstop_conf_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.conf, file = paste0(prediction_path,"busstop_conf_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
...
#write.table(df.acc, file = paste0(prediction_path,"busstop_acc_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#write.table(df.acc, file = paste0(prediction_path,"busstop_acc_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
write.table(df.acc, file = paste0(prediction_path,"busstop_acc_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.acc, file = paste0(prediction_path,"busstop_acc_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
...
ggsave(paste0(figure_path,"busstop_use_upsampled_timecourse.png"), dpi = 300, scale = 1)
...
ggsave(paste0(figure_path,"busstop_confidence_upsampled_timecourse.png"), dpi = 300, scale = 1)
...
ggplot(data=tr, aes(x=s, y=accuracy)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("accuracy") + ylim(.2, .7)
ggsave(paste0(figure_path,"busstop_accuracy_upsampled_timecourse.png"), dpi = 300, scale = 1)


#NOTE:should be able to run everything except roi course
...
setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/busstop/roi_tc/tc")
temp = list.files(pattern="*.txt")
tc_list = lapply(temp, function(x) read.delim(x, header = FALSE, sep = ","))
#load group averaged pISC ROI timecourses
setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/busstop/roi_tc/pisc")
...
for (i in 1:length(tc_list)){
  ...
  #set path
  setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/busstop/roi_tc/")
  ...
  ggsave(paste0("busstop_",stri_sub(stri_sub(temp[i],12),1,-16),"_timecourses.png"), scale = 2)
