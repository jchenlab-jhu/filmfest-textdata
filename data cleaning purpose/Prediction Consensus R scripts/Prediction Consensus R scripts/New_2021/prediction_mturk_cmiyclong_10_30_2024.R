...
setwd("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/cmiyc_long/")

#create additional directories
prediction_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/cmiyc_long/predictions/"
dir.create(prediction_path, showWarnings = FALSE)
figure_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/cmiyc_long/figures/"
dir.create(figure_path, showWarnings = FALSE)
cosmat_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/cmiyc_long/figures/cosmat/"
dir.create(cosmat_path, showWarnings = FALSE)
hist_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/cmiyc_long/figures/hist/"
dir.create(hist_path, showWarnings = FALSE)
clean_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/cmiyc_long/predictions/clean/"
dir.create(clean_path, showWarnings = FALSE)
raw_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/cmiyc_long/predictions/raw/"
dir.create(raw_path, showWarnings = FALSE)

##load data
#contains trial data & keywords
trial.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/cmiyc_long/trialdata.csv", header = FALSE, comment.char="#")
colnames(trial.data) <- c("ID","Trial","Timestamp","Datastring")
#contains post-task survey data
question.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/cmiyc_long/questiondata.csv", header = FALSE, comment.char="#")
colnames(question.data) <- c("ID","Question","Response")
#contains window events (e.g., focus on/off, window resize, etc)
event.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/cmiyc_long/eventdata.csv", header = FALSE, comment.char="#")
...
#loop checking section -> use this to create list to drop for participants that looped (see Exp Repeat IDs README)
...
#final drop list of participants affected by looping issue
#keeping participants that only looped instructions and not actual experiment
drop.loops <- c("debugNSQx8:debugypdvf",
                "debugm3tNI:debug9KzKf")

trial.data <- subset(trial.data, !(turkID %in% drop.loops))
question.data <- subset(question.data, !(turkID %in% drop.loops))
event.data <- subset(event.data, !(turkID %in% drop.loops))
...
comprehension <- within(comprehension, {
  question_text = ifelse(comprehension$Question=="Comp_Q1", "What was the title of this clip, presented in a white font on a black background?", ifelse(comprehension$Question== "Comp_Q2","Which of the following best describes the weather in the prison scenes?", ifelse(comprehension$Question== "Comp_Q3", "Whhich of the following best describes what followed the prison scene?", ifelse(comprehension$Question== "Comp_Q4", "What were the characters doing when they were back at home?", "ERROR"))))
})
...
spoiler.data$dataset <- "cmiyc_long"
#drop turkID from question.data
question.data <- subset(question.data, select= -c(turkID))
#drop ID from spoiler.data
spoiler.data <- subset(spoiler.data, select= -c(ID))
#total number of subjects
length(unique(spoiler.data$turkID))
#export spoiler questions as csv
write.csv(spoiler.data, file = paste0(raw_path,"cmiyc_long_spoilerdata.csv")) #save dataframe to csv
#drop turkID from trial.data
trial.data <- subset(trial.data, select= -c(turkID))
...
prediction.data$Datastring <- as.character(prediction.data$Datastring)

#Removing delimiter in prediction content to prevent scrambling the data table (participant accidentally entered delimiter in their input)
prediction.data$Datastring <- gsub("Frank is inspired by his Father, ", "Frank is inspired by his Father,", prediction.data$Datastring)
prediction.data$Datastring <- gsub("The television host will ask the contestant which of the three men really is Frank William Abignale, ", "The television host will ask the contestant which of the three men really is Frank William Abignale,", prediction.data$Datastring)

#parse into segments
temp <- do.call(rbind, strsplit(prediction.data$Datastring, ", \"|(>\\\".*?\\\".*?\\K(, |$))", perl = TRUE)) 
...
temp2$V2 <- prediction.data$ID
temp2$V3 <- prediction.data$Trial
...
#rename columns
colnames(temp4) <- c("confidence", "prediction_content", "counterbalance", "offset", "phase", "prediction_number", "video_segment", "scratch", "time", "vidpath", "tag", "ID", "Trial")
temp <- temp4
...
prediction.data$time <- stri_replace_all_charclass(prediction.data$time, "\\p{WHITE_SPACE}", "")
...
prediction.data$confidence <- stri_sub(prediction.data$confidence,12) #remove 12 characters from beginning
...
prediction.data$time <- gsub('"', '', prediction.data$time) #remove quotes
prediction.data$time <- stri_sub(prediction.data$time,6) #remove 6 characters from beginning
...
prediction.data$tag <- stri_sub(prediction.data$tag,1,-2) #remove 1 characters from end
...
export_clean_prediction <- function(df,phase,counterbalance,segment){
  ...
  write.table(df, file = paste0(clean_path,"cmiyc_long_",phase,"_c",counterbalance,"_seg",segment,".txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to csv
}
...
export_data_frame <- function(df) {
  df2 <- df %>%
    add_column(data_set = "2") # 2 = new (2021) data
  saveRDS(df2, file = "new_data.Rds")
}
...
test_cAllsegTitle <- prediction.data[grep("cmiyc_long_title_4s.mp4", prediction.data$vidpath), ]
...
test_c0s4 <- subset(prediction.data, counterbalance == 0 & video_segment == "4", select = c(prediction_content))
export_clean_prediction(test_c0s4, "test", "0", "4")
test_c0s5 <- subset(prediction.data, counterbalance == 0 & video_segment == "5", select = c(prediction_content))
export_clean_prediction(test_c0s5, "test", "0", "5")
test_c0s6 <- subset(prediction.data, counterbalance == 0 & video_segment == "6", select = c(prediction_content))
export_clean_prediction(test_c0s6, "test", "0", "6")
test_c0s7 <- subset(prediction.data, counterbalance == 0 & video_segment == "7", select = c(prediction_content))
export_clean_prediction(test_c0s7, "test", "0", "7")
test_c0s8 <- subset(prediction.data, counterbalance == 0 & video_segment == "8", select = c(prediction_content))
export_clean_prediction(test_c0s8, "test", "0", "8")
test_c0s9 <- subset(prediction.data, counterbalance == 0 & video_segment == "9", select = c(prediction_content))
export_clean_prediction(test_c0s9, "test", "0", "9")
test_c0s10 <- subset(prediction.data, counterbalance == 0 & video_segment == "10", select = c(prediction_content))
export_clean_prediction(test_c0s10, "test", "0", "10")
test_c0s11 <- subset(prediction.data, counterbalance == 0 & video_segment == "11", select = c(prediction_content))
export_clean_prediction(test_c0s11, "test", "0", "11")
test_c0s12 <- subset(prediction.data, counterbalance == 0 & video_segment == "12", select = c(prediction_content))
export_clean_prediction(test_c0s12, "test", "0", "12")
#counterbalance: 1
...
test_c1s4 <- subset(prediction.data, counterbalance == 1 & video_segment == "4", select = c(prediction_content))
export_clean_prediction(test_c1s4, "test", "1", "4")
test_c1s5 <- subset(prediction.data, counterbalance == 1 & video_segment == "5", select = c(prediction_content))
export_clean_prediction(test_c1s5, "test", "1", "5")
test_c1s6 <- subset(prediction.data, counterbalance == 1 & video_segment == "6", select = c(prediction_content))
export_clean_prediction(test_c1s6, "test", "1", "6")
test_c1s7 <- subset(prediction.data, counterbalance == 1 & video_segment == "7", select = c(prediction_content))
export_clean_prediction(test_c1s7, "test", "1", "7")
test_c1s8 <- subset(prediction.data, counterbalance == 1 & video_segment == "8", select = c(prediction_content))
export_clean_prediction(test_c1s8, "test", "1", "8")
test_c1s9 <- subset(prediction.data, counterbalance == 1 & video_segment == "9", select = c(prediction_content))
export_clean_prediction(test_c1s9, "test", "1", "9")
test_c1s10 <- subset(prediction.data, counterbalance == 1 & video_segment == "10", select = c(prediction_content))
export_clean_prediction(test_c1s10, "test", "1", "10")
test_c1s11 <- subset(prediction.data, counterbalance == 1 & video_segment == "11", select = c(prediction_content))
export_clean_prediction(test_c1s11, "test", "1", "11")
test_c1s12 <- subset(prediction.data, counterbalance == 1 & video_segment == "12", select = c(prediction_content))
export_clean_prediction(test_c1s12, "test", "1", "12")
#counterbalance: 2
...
test_c2s4 <- subset(prediction.data, counterbalance == 2 & video_segment == "4", select = c(prediction_content))
export_clean_prediction(test_c2s4, "test", "2", "4")
test_c2s5 <- subset(prediction.data, counterbalance == 2 & video_segment == "5", select = c(prediction_content))
export_clean_prediction(test_c2s5, "test", "2", "5")
test_c2s6 <- subset(prediction.data, counterbalance == 2 & video_segment == "6", select = c(prediction_content))
export_clean_prediction(test_c2s6, "test", "2", "6")
test_c2s7 <- subset(prediction.data, counterbalance == 2 & video_segment == "7", select = c(prediction_content))
export_clean_prediction(test_c2s7, "test", "2", "7")
test_c2s8 <- subset(prediction.data, counterbalance == 2 & video_segment == "8", select = c(prediction_content))
export_clean_prediction(test_c2s8, "test", "2", "8")
test_c2s9 <- subset(prediction.data, counterbalance == 2 & video_segment == "9", select = c(prediction_content))
export_clean_prediction(test_c2s9, "test", "2", "9")
test_c2s10 <- subset(prediction.data, counterbalance == 2 & video_segment == "10", select = c(prediction_content))
export_clean_prediction(test_c2s10, "test", "2", "10")
test_c2s11 <- subset(prediction.data, counterbalance == 2 & video_segment == "11", select = c(prediction_content))
export_clean_prediction(test_c2s11, "test", "2", "11")
test_c2s12 <- subset(prediction.data, counterbalance == 2 & video_segment == "12", select = c(prediction_content))
export_clean_prediction(test_c2s12, "test", "2", "12")
#counterbalance: 3
...
test_c3s4 <- subset(prediction.data, counterbalance == 3 & video_segment == "4", select = c(prediction_content))
export_clean_prediction(test_c3s4, "test", "3", "4")
test_c3s5 <- subset(prediction.data, counterbalance == 3 & video_segment == "5", select = c(prediction_content))
export_clean_prediction(test_c3s5, "test", "3", "5")
test_c3s6 <- subset(prediction.data, counterbalance == 3 & video_segment == "6", select = c(prediction_content))
export_clean_prediction(test_c3s6, "test", "3", "6")
test_c3s7 <- subset(prediction.data, counterbalance == 3 & video_segment == "7", select = c(prediction_content))
export_clean_prediction(test_c3s7, "test", "3", "7")
test_c3s8 <- subset(prediction.data, counterbalance == 3 & video_segment == "8", select = c(prediction_content))
export_clean_prediction(test_c3s8, "test", "3", "8")
test_c3s9 <- subset(prediction.data, counterbalance == 3 & video_segment == "9", select = c(prediction_content))
export_clean_prediction(test_c3s9, "test", "3", "9")
test_c3s10 <- subset(prediction.data, counterbalance == 3 & video_segment == "10", select = c(prediction_content))
export_clean_prediction(test_c3s10, "test", "3", "10")
test_c3s11 <- subset(prediction.data, counterbalance == 3 & video_segment == "11", select = c(prediction_content))
export_clean_prediction(test_c3s11, "test", "3", "11")
test_c3s12 <- subset(prediction.data, counterbalance == 3 & video_segment == "12", select = c(prediction_content))
export_clean_prediction(test_c3s12, "test", "3", "12")
#counterbalance: 4
...
test_c4s4 <- subset(prediction.data, counterbalance == 4 & video_segment == "4", select = c(prediction_content))
export_clean_prediction(test_c4s4, "test", "4", "4")
test_c4s5 <- subset(prediction.data, counterbalance == 4 & video_segment == "5", select = c(prediction_content))
export_clean_prediction(test_c4s5, "test", "4", "5")
test_c4s6 <- subset(prediction.data, counterbalance == 4 & video_segment == "6", select = c(prediction_content))
export_clean_prediction(test_c4s6, "test", "4", "6")
test_c4s7 <- subset(prediction.data, counterbalance == 4 & video_segment == "7", select = c(prediction_content))
export_clean_prediction(test_c4s7, "test", "4", "7")
test_c4s8 <- subset(prediction.data, counterbalance == 4 & video_segment == "8", select = c(prediction_content))
export_clean_prediction(test_c4s8, "test", "4", "8")
test_c4s9 <- subset(prediction.data, counterbalance == 4 & video_segment == "9", select = c(prediction_content))
export_clean_prediction(test_c4s9, "test", "4", "9")
test_c4s10 <- subset(prediction.data, counterbalance == 4 & video_segment == "10", select = c(prediction_content))
export_clean_prediction(test_c4s10, "test", "4", "10")
test_c4s11 <- subset(prediction.data, counterbalance == 4 & video_segment == "11", select = c(prediction_content))
export_clean_prediction(test_c4s11, "test", "4", "11")
test_c4s12 <- subset(prediction.data, counterbalance == 4 & video_segment == "12", select = c(prediction_content))
export_clean_prediction(test_c4s12, "test", "4", "12")
#counterbalance: 5
...
test_c5s4 <- subset(prediction.data, counterbalance == 5 & video_segment == "4", select = c(prediction_content))
export_clean_prediction(test_c5s4, "test", "5", "4")
test_c5s5 <- subset(prediction.data, counterbalance == 5 & video_segment == "5", select = c(prediction_content))
export_clean_prediction(test_c5s5, "test", "5", "5")
test_c5s6 <- subset(prediction.data, counterbalance == 5 & video_segment == "6", select = c(prediction_content))
export_clean_prediction(test_c5s6, "test", "5", "6")
test_c5s7 <- subset(prediction.data, counterbalance == 5 & video_segment == "7", select = c(prediction_content))
export_clean_prediction(test_c5s7, "test", "5", "7")
test_c5s8 <- subset(prediction.data, counterbalance == 5 & video_segment == "8", select = c(prediction_content))
export_clean_prediction(test_c5s8, "test", "5", "8")
test_c5s9 <- subset(prediction.data, counterbalance == 5 & video_segment == "9", select = c(prediction_content))
export_clean_prediction(test_c5s9, "test", "5", "9")
test_c5s10 <- subset(prediction.data, counterbalance == 5 & video_segment == "10", select = c(prediction_content))
export_clean_prediction(test_c5s10, "test", "5", "10")
test_c5s11 <- subset(prediction.data, counterbalance == 5 & video_segment == "11", select = c(prediction_content))
export_clean_prediction(test_c5s11, "test", "5", "11")
test_c5s12 <- subset(prediction.data, counterbalance == 5 & video_segment == "12", select = c(prediction_content))
export_clean_prediction(test_c5s12, "test", "5", "12")
...
ggsave(paste0(figure_path,"cmiyc_use_timecourse.png"), dpi = 300, scale = 1)
...
ggplot(data=use.data, aes(x=offset, y=similarity)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("cosine similarity") + ylim(-.1, .8)
ggsave(paste0(figure_path,"cmiyc_use_timecourse_labelled.png"), dpi = 300, scale = 1)
...
ggsave(paste0(figure_path,"cmiyc_confidence_timecourse.png"), dpi = 300, scale = 1)
...
ggplot(data=confidence.data, aes(x=offset, y=z_mean)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("confidence (z)") + ylim(-1.1, 1.5)
ggsave(paste0(figure_path,"cmiyc_confidence_timecourse_labelled.png"), dpi = 300, scale = 1)
...
ggsave(paste0(figure_path,"cmiyc_accuracy_timecourse.png"), dpi = 600, scale = 1)
...
confidence.data$experiment <- "cmiyc_long"
use.data$experiment <- "cmiyc_long"
accuracy.data$experiment <- "cmiyc_long"
...
ggplot(confidence.data,aes(x=z_mean, y=use.data$similarity))+
  ...
  labs(x = "confidence (z)", y = "consensus") + 
...
ggsave(paste0(figure_path,"cmiyc_use_conf_scatter.png"), scale = 1)
...
ggplot(confidence.data,aes(x=z_mean, y=accuracy.data$similarity))+
  ...
  scale_x_continuous(limits = c(-.8,.8)) +
  stat_cor(method = "pearson", label.x = (median((-8:8)/10)), label.y = .675) #from ggpubr -- plots r and p-value
ggsave(paste0(figure_path,"cmiyc_accuracy_conf_scatter.png"), scale = 1)
...
ggplot(accuracy.data,aes(x=similarity, y=use.data$similarity))+
  ...
  stat_cor(method = "pearson", label.x = .4, label.y = .675) #from ggpubr -- plots r and p-value
ggsave(paste0(figure_path,"cmiyc_accuracy_use_scatter.png"), scale = 1)
...
#build a vector representing each TR, in seconds, for the CMIYC movie
tr <- as.data.frame(seq(from = 0, to = 665, by = 1.5))
#tr <- as.data.frame(seq(from = 0, to = 351, by = 1.5)) #for filmfest version (shorter than CMIYC_long)
...
write.table(df.orig.use, file = paste0(prediction_path,"cmiyc_long_use_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.use, file = paste0(prediction_path,"cmiyc_long_use_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
df.orig.conf <- subset(confidence.data, select = z_mean)
write.table(df.orig.conf, file = paste0(prediction_path,"cmiyc_long_conf_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.conf, file = paste0(prediction_path,"cmiyc_long_conf_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
df.orig.acc <- subset(accuracy.data, select = similarity)
write.table(df.orig.acc, file = paste0(prediction_path,"cmiyc_long_acc_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.acc, file = paste0(prediction_path,"cmiyc_long_acc_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#raw, not-interpolated, filmfest version
df.orig.use <- subset(use.data, offset<351, select = similarity)
write.table(df.orig.use, file = paste0(prediction_path,"cmiyc_short_use_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.use, file = paste0(prediction_path,"cmiyc_short_use_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
df.orig.conf <- subset(confidence.data, offset<351, select = z_mean)
write.table(df.orig.conf, file = paste0(prediction_path,"cmiyc_short_conf_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.conf, file = paste0(prediction_path,"cmiyc_short_conf_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
df.orig.acc <- subset(accuracy.data, offset<351, select = similarity)
write.table(df.orig.acc, file = paste0(prediction_path,"cmiyc_short_acc_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.acc, file = paste0(prediction_path,"cmiyc_short_acc_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv

#write.csv(tr, file = paste0(prediction_path,"cmiyc_long_full_interp_tr_match.csv")) #save full dataframe to csv
df.use <- subset(tr, select = similarity)
df.use <- df.use[15:length(t(df.use)),] #remove first 15 trs (to deal with an intial transient in the BOLD data)
#df.use <- df.use[2:length(t(df.use)),] #remove first tr only
write.table(df.use, file = paste0(prediction_path,"cmiyc_long_use_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.use, file = paste0(prediction_path,"cmiyc_long_use_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
#write.table(df.use, file = paste0(prediction_path,"cmiyc_long_use_443tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#write.table(df.use, file = paste0(prediction_path,"cmiyc_long_use_443tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
#write.table(df.use, file = paste0(prediction_path,"cmiyc_long_use_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#write.table(df.use, file = paste0(prediction_path,"cmiyc_long_use_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
df.conf <- subset(tr, select = confidence)
df.conf <- df.conf[15:length(t(df.conf)),] #remove first 15 trs (to deal with an intial transient in the BOLD data)
#df.conf <- df.conf[2:length(t(df.conf)),] #remove first tr only
write.table(df.conf, file = paste0(prediction_path,"cmiyc_long_conf_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.conf, file = paste0(prediction_path,"cmiyc_long_conf_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
#write.table(df.conf, file = paste0(prediction_path,"cmiyc_long_conf_443tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#write.table(df.conf, file = paste0(prediction_path,"cmiyc_long_conf_443tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
#write.table(df.conf, file = paste0(prediction_path,"cmiyc_long_conf_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
df.acc <- subset(tr, select = accuracy)
df.acc <- df.acc[15:length(t(df.acc)),] #remove first 15 trs (to deal with an intial transient in the BOLD data)
#df.acc <- df.acc[2:length(t(df.acc)),] #remove first tr only
write.table(df.acc, file = paste0(prediction_path,"cmiyc_long_acc_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.acc, file = paste0(prediction_path,"cmiyc_long_acc_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
#write.table(df.acc, file = paste0(prediction_path,"cmiyc_long_acc_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#write.table(df.acc, file = paste0(prediction_path,"cmiyc_long_acc_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
#write.table(df.acc, file = paste0(prediction_path,"cmiyc_long_acc_443tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#write.table(df.acc, file = paste0(prediction_path,"cmiyc_long_acc_443tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
...
ggsave(paste0(figure_path,"cmiyc_use_upsampled_timecourse.png"), dpi = 600, scale = 1)
...
ggsave(paste0(figure_path,"cmiyc_confidence_upsampled_timecourse.png"), dpi = 600, scale = 1)
...
ggplot(data=tr, aes(x=s, y=accuracy)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("accuracy") + ylim(.2, .7)
ggsave(paste0(figure_path,"cmiyc_accuracy_upsampled_timecourse.png"), dpi = 600, scale = 1)
...
setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/cmiyc_long/roi_tc/tc")
#setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/cmiyc_short/roi_tc/tc")
temp = list.files(pattern="*.txt")
tc_list = lapply(temp, function(x) read.delim(x, header = FALSE, sep = ","))
#load group averaged pISC ROI timecourses
setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/cmiyc_long/roi_tc/pisc")
#setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/cmiyc_short/roi_tc/pisc")
...
tr_long$confidence <- as.numeric(scale(tr_long$confidence)) #zscore confidence    
...
for (i in 1:length(tc_list)){
  ...
  #set path
  setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/cmiyc_long/roi_tc/")
  #setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/cmiyc_short/roi_tc/")
  ...
  ggsave(paste0("cmiyc_",stri_sub(stri_sub(temp[i],12),1,-16),"_timecourses.png"), scale = 2)
...
#raw, not-interpolated, filmfest version
df.orig.count.cluster <- subset(rater.summary, offset<351, select = count.cluster)
write.table(df.orig.count.cluster, file = paste0(prediction_path,"cmiyc_short_countcluster_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.count.cluster, file = paste0(prediction_path,"cmiyc_short_countcluster_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
df.orig.percent.max <- subset(rater.summary, offset<351, select = percent.max)
write.table(df.orig.percent.max, file = paste0(prediction_path,"cmiyc_short_percentmax_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.percent.max, file = paste0(prediction_path,"cmiyc_short_percentmax_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
df.orig.percent.mean <- subset(rater.summary, offset<351, select = percent.mean)
write.table(df.orig.percent.mean, file = paste0(prediction_path,"cmiyc_short_percentmean_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.percent.mean, file = paste0(prediction_path,"cmiyc_short_percentmean_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv

#interpolated data
write.csv(tr, file = paste0(prediction_path,movie,"_full_interp_tr_match.csv")) #save full dataframe to csv
#write.csv(tr, file = paste0(prediction_path,movie,"_full_interp_tr_match_short.csv")) #save full dataframe to csv

df.count.cluster <- subset(tr, select = count.cluster)
df.count.cluster <- df.count.cluster[2:length(t(df.count.cluster)),] #remove first tr only
...
#for when tr is set to 351 (length of the filmfest version of cmiyc)
...
write.table(df.count.cluster, file = paste0(prediction_path,movie,"_countcluster_443tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.count.cluster, file = paste0(prediction_path,movie,"_countcluster_443tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt

df.percent.max <- subset(tr, select = percent.max)
df.percent.max <- df.percent.max[2:length(t(df.percent.max)),] #remove first tr only
...
write.table(df.percent.max, file = paste0(prediction_path,movie,"_percentmax_443tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.percent.max, file = paste0(prediction_path,movie,"_percentmax_443tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt

df.percent.mean <- subset(tr, select = percent.mean)
df.percent.mean <- df.percent.mean[2:length(t(df.percent.mean)),] #remove first tr only
...
write.table(df.percent.mean, file = paste0(prediction_path,movie,"_percentmean_443tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.percent.mean, file = paste0(prediction_path,movie,"_percentmean_443tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt


#####look at reliability across raters#####
#note: need to run the above section for each rater, and save the rater.summary.emh variable (EMH's rating) so it is not overwritten. then, you can compare the two raters
#run correlation tests
...
ggplot(rater.summary,aes(x=percent.mean, y=rater.summary.emh$percent.mean))+
  ...
  stat_cor(method = "pearson", label.x = (median(0:40)), label.y = 35) #from ggpubr -- plots r and p-value
ggsave(paste0(figure_path,movie,"_human_mean_interrater_reliability.png"), scale = 1)
...
ggplot(rater.summary,aes(x=percent.max, y=rater.summary.emh$percent.max))+
  ...
  stat_cor(method = "pearson", label.x = (median(0:100)), label.y = 90) #from ggpubr -- plots r and p-value
ggsave(paste0(figure_path,movie,"_human_max_interrater_reliability.png"), scale = 1)
...
ggplot(rater.summary,aes(x=count.cluster, y=rater.summary.emh$count.cluster))+
  ...
  scale_x_continuous(limits = c(1,20)) +
  stat_cor(method = "pearson", label.x = (median(1:20)), label.y = 20) #from ggpubr -- plots r and p-value
ggsave(paste0(figure_path,movie,"_human_count_interrater_reliability.png"), scale = 1)
...
####################################################################################
  ...
  common_time <- as.data.frame(seq(from = 0, to = 665, by = .5))
  #common_time<- as.data.frame(seq(from = 0, to = 351, by = .5)) #for filmfest version (shorter than CMIYC_long)
...
#################################################################
  #load annotations
  annot_cmiyc_long_JL <- read.csv("~/Dropbox/Academic/Projects/JHU/FilmFest/behaviour/FilmFest_Annotation_JL_CMIYC_Extended_KW.csv")
  names(annot_cmiyc_long_JL)
  
  #clean up
  annot_cmiyc_long_JL <- subset(annot_cmiyc_long_JL, select = -c(X,X.1,X.2,X.3,X.4)) #remove unnecessary columns
  annot_cmiyc_long_JL <- head(annot_cmiyc_long_JL,tail(which(annot_cmiyc_long_JL$SEG.1000.number!=0),1)) #remove blank rows
  colnames(annot_cmiyc_long_JL)[which(names(annot_cmiyc_long_JL) == "Start.Time..m.ss.")] <- "SEG.216.Start.Time..m.ss."
  colnames(annot_cmiyc_long_JL)[which(names(annot_cmiyc_long_JL) == "End.Time..m.ss.")] <- "SEG.216.End.Time..m.ss."
  ...
  #SEG.12.title
  while(length(ind <- which(annot_cmiyc_long_JL$SEG.12.title  == "")) > 0){
    annot_cmiyc_long_JL$SEG.12.title[ind] <- annot_cmiyc_long_JL$SEG.12.title[ind -1]
  }
  #SEG.216.number
  while(length(ind <- which(is.na(annot_cmiyc_long_JL$SEG.216.number))) > 0){
    annot_cmiyc_long_JL$SEG.216.number[ind] <- annot_cmiyc_long_JL$SEG.216.number[ind -1]
  }
  #SEG.216.Start.Time..m.ss.
  while(length(ind <- which(is.na(annot_cmiyc_long_JL$SEG.216.Start.Time..m.ss.))) > 0){
    annot_cmiyc_long_JL$SEG.216.Start.Time..m.ss.[ind] <- annot_cmiyc_long_JL$SEG.216.Start.Time..m.ss.[ind -1]
  }
  #SEG.216.End.Time..m.ss.
  while(length(ind <- which(is.na(annot_cmiyc_long_JL$SEG.216.End.Time..m.ss.))) > 0){
    annot_cmiyc_long_JL$SEG.216.End.Time..m.ss.[ind] <- annot_cmiyc_long_JL$SEG.216.End.Time..m.ss.[ind -1]
  }
  #SEG.216.Description
  while(length(ind <- which(annot_cmiyc_long_JL$SEG.216.Description  == "")) > 0){
    annot_cmiyc_long_JL$SEG.216.Description[ind] <- annot_cmiyc_long_JL$SEG.216.Description[ind -1]
  }
  ...
  #for SEG.216
  annot_cmiyc_long_JL$SEG.216.Start.S <- (floor(annot_cmiyc_long_JL$SEG.216.Start.Time..m.ss.)*60)+(reverse_truncate(annot_cmiyc_long_JL$SEG.216.Start.Time..m.ss.)*100)
  annot_cmiyc_long_JL$SEG.216.End.S <- (floor(annot_cmiyc_long_JL$SEG.216.End.Time..m.ss.)*60)+(reverse_truncate(annot_cmiyc_long_JL$SEG.216.End.Time..m.ss.)*100)
  #for SEG.1000
  annot_cmiyc_long_JL$SEG.1000.Start.S <- (floor(annot_cmiyc_long_JL$SEG.1000.Start.Time..m.ss.)*60)+(reverse_truncate(annot_cmiyc_long_JL$SEG.1000.Start.Time..m.ss.)*100)
  annot_cmiyc_long_JL$SEG.1000.End.S <- (floor(annot_cmiyc_long_JL$SEG.1000.End.Time..m.ss.)*60)+(reverse_truncate(annot_cmiyc_long_JL$SEG.1000.End.Time..m.ss.)*100)
  
  #isolate CMIYC only (i.e., drop the "Let's all go to the lobby" clip) and start timings at actual movie onsets
  annot_cmiyc_long_clean <- subset(annot_cmiyc_long_JL, !SEG.12.title %in% c("1. Lobby 1", "1. Lobby 2"))
  #reset times to beginning of CMIYC
  movie_start_time <- head(annot_cmiyc_long_clean$SEG.216.Start.S ,1) #define movie start time
  annot_cmiyc_long_clean$SEG.216.Start.S <- annot_cmiyc_long_clean$SEG.216.Start.S - movie_start_time
  annot_cmiyc_long_clean$SEG.216.End.S <- annot_cmiyc_long_clean$SEG.216.End.S - movie_start_time
  annot_cmiyc_long_clean$SEG.1000.Start.S <- annot_cmiyc_long_clean$SEG.1000.Start.S - movie_start_time
  annot_cmiyc_long_clean$SEG.1000.End.S <- annot_cmiyc_long_clean$SEG.1000.End.S - movie_start_time
  
  #take annotation data and add to common_times
  #create empty variables
  common_time$SEG216_number <- 0
  common_time$SEG1000_number <- 0
  ...
  #loop through TRs to find corresponding offsets in the interpolated timecourse
  for (i in 1:length(common_time$s)){
    common_time$position_match[i] <- if(is.finite(min(annot_cmiyc_long_clean$SEG.1000.End.S[annot_cmiyc_long_clean$SEG.1000.End.S >= common_time$s[i]]))==FALSE){NA}else{min(annot_cmiyc_long_clean$SEG.1000.End.S[annot_cmiyc_long_clean$SEG.1000.End.S >= common_time$s[i]])} #find closest offset in use.data that is greater than or equal to common_time$s[i]
    common_time$SEG1000_number[i] <- as.numeric(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$SEG.1000.number)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$SEG.1000.number}) #find corresponding SEG1000 number 
    common_time$SEG216_number[i] <- as.numeric(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$SEG.216.number)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$SEG.216.number}) #find corresponding SEG216 number 
    common_time$SEG1000_description[i] <- as.character(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$SEG.1000.Description)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$SEG.1000.Description}) #find corresponding SEG1000 description 
    common_time$SEG216_description[i] <- as.character(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$SEG.216.Description)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$SEG.216.Description}) #find corresponding similarity SEG216 description 
    common_time$SEG1000_valence[i] <- as.numeric(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Valence)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Valence}) #find corresponding SEG1000 valence 
    common_time$SEG1000_arousal[i] <- as.numeric(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Arousal)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Arousal}) #find corresponding SEG1000 arousal 
  }
  ...
  for (i in 1:length(common_time$s)){
    ...
    common_time$SEG1000_valence[i] <- as.numeric(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Valence)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Valence}) #find corresponding SEG1000 valence 
    common_time$SEG1000_arousal[i] <- as.numeric(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Arousal)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Arousal}) #find corresponding SEG1000 arousal 
    common_time$SEG1000_indoor_outdoor[i] <- as.character(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Space.In.Outdoor)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Space.In.Outdoor}) #find corresponding SEG1000 Space.In.Outdoor label
    common_time$SEG1000_location[i] <- as.character(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Location)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Location}) #find corresponding SEG1000 Location label
    common_time$SEG1000_onscreen_chars[i] <- as.character(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Name...Onscreen)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Name...Onscreen}) #find corresponding SEG1000 Name...Onscreen label
    common_time$SEG1000_present_chars[i] <- as.character(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Name...Present)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Name...Present}) #find corresponding SEG1000 Name...Present label
    common_time$SEG1000_speaking_chars[i] <- as.character(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Name...Speaking)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Name...Speaking}) #find corresponding SEG1000 Name...Speaking label
    common_time$SEG1000_music[i] <- as.character(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Music.Presence)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Music.Presence}) #find corresponding SEG1000 Music.Presence label
    common_time$SEG1000_onscreen_words[i] <- as.character(if(is.finite(annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Words.on.Screen)==FALSE){NA}else{annot_cmiyc_long_clean[common_time$position_match[i]==annot_cmiyc_long_clean$SEG.1000.End.S,]$Words.on.Screen}) #find corresponding SEG1000 Words.on.Screen label
  }
  
  #add boundary strength ratings, from beginning of event, to specific boundary points
  common_time$SEG1000_boundarystrength<- with(annot_cmiyc_long_clean, SEG.1000.boundary.strength[match(common_time$s, SEG.1000.Start.S)])
  ...
  for (i in 1:length(common_time$s)){
    ...
    common_time$SEG1000_start[i] <- if(length(common_time$SEG1000_number[i-1]>0)){(if(common_time$SEG1000_number[i]==common_time$SEG1000_number[i-1]){0}else{1})}else{1}#add 1 to rows when a SEG1000 event starts and add 0s everywhere else
    common_time$SEG1000_end[i] <- if(is.na(common_time$SEG1000_number[i+1])==FALSE){(if(common_time$SEG1000_number[i]==common_time$SEG1000_number[i+1]){0}else{1})}else{1}#add 1 to rows when a SEG1000 event starts and add 0s everywhere else
  }
...
##############################################################
  #load annotations
  importance_cmiyc_ff <- read.csv("~/Dropbox/Academic/Projects/JHU/FilmFest/behaviour/Hongmi_behav_data/FilmFest_Combined_Importance_Sep262019.csv")
  names(importance_cmiyc_ff)

  #clean up
  importance_cmiyc <- tail(head(importance_cmiyc_ff, 26),24) #manually specify where the CMIYC data live in the dataframe
  importance_cmiyc <- subset(importance_cmiyc, select = -c(End.Time..s.,End.Time..TRs..1.5s.,Start.Time..s.....concatenated,Start.Time..TRs..1.5s....concatenated,Start.Time..s.,Start.Time..TRs..1.5s.,scene.number...concatenated,End.Time..s.....concatenated,End.Time..TRs..1.5s....concatenated,Start.Time..m.ss.)) #remove unnecessary columns
  colnames(importance_cmiyc)[which(names(importance_cmiyc) == "scene.start.time")] <- "SEG.216.Start.Time..m.ss."
  colnames(importance_cmiyc)[which(names(importance_cmiyc) == "scene.end.time")] <- "SEG.216.End.Time..m.ss."
  #turn times (e.g., 1:10) to decimals (e.g., 1.10)
  importance_cmiyc$SEG.216.Start.Time..m.ss. <- as.character(importance_cmiyc$SEG.216.Start.Time..m.ss.)
  importance_cmiyc$SEG.216.Start.Time..m.ss. <- gsub(":", ".", importance_cmiyc$SEG.216.Start.Time..m.ss.)
  importance_cmiyc$SEG.216.Start.Time..m.ss. <- as.numeric(importance_cmiyc$SEG.216.Start.Time..m.ss.)
  importance_cmiyc$SEG.216.End.Time..m.ss. <- as.character(importance_cmiyc$SEG.216.End.Time..m.ss.)
  importance_cmiyc$SEG.216.End.Time..m.ss. <- gsub(":", ".", importance_cmiyc$SEG.216.End.Time..m.ss.)
  importance_cmiyc$SEG.216.End.Time..m.ss. <- as.numeric(importance_cmiyc$SEG.216.End.Time..m.ss.)
  names(importance_cmiyc)
  
  #turn minute.seconds into total seconds
  #for SEG.216
  importance_cmiyc$SEG.216.Start.S <- (floor(importance_cmiyc$SEG.216.Start.Time..m.ss.)*60)+(reverse_truncate(importance_cmiyc$SEG.216.Start.Time..m.ss.)*100)
  importance_cmiyc$SEG.216.End.S <- (floor(importance_cmiyc$SEG.216.End.Time..m.ss.)*60)+(reverse_truncate(importance_cmiyc$SEG.216.End.Time..m.ss.)*100)
  #reset times to beginning of CMIYC
  movie_start_time==head(importance_cmiyc$SEG.216.Start.S,1) #make sure movie_start_time matches across datasets
  importance_cmiyc$SEG.216.Start.S <- importance_cmiyc$SEG.216.Start.S - movie_start_time
  importance_cmiyc$SEG.216.End.S <- importance_cmiyc$SEG.216.End.S - movie_start_time
  
  #scale all importance ratings within subject
  importance_cmiyc$Importance_Rater1_PostView <- scale(importance_cmiyc$Importance_Rater1_PostView)
  importance_cmiyc$Importance_Rater2_PostView <- scale(importance_cmiyc$Importance_Rater2_PostView)
  importance_cmiyc$Importance_Rater3_PostView <- scale(importance_cmiyc$Importance_Rater3_PostView)
  importance_cmiyc$Importance_Rater4_PostView <- scale(importance_cmiyc$Importance_Rater4_PostView)
  importance_cmiyc$Importance_Rater1_Online <- scale(importance_cmiyc$Importance_Rater1_Online)
  importance_cmiyc$Importance_Rater2_Online <- scale(importance_cmiyc$Importance_Rater2_Online)
  importance_cmiyc$Importance_Rater3_Online <- scale(importance_cmiyc$Importance_Rater3_Online)
  importance_cmiyc$Importance_Rater4_Online <- scale(importance_cmiyc$Importance_Rater4_Online)
  
  #average importance ratings within condition (postview, online)
  importance_cmiyc$mean_importance_postview <- rowMeans(subset(importance_cmiyc, select = c(Importance_Rater1_PostView,Importance_Rater2_PostView,Importance_Rater3_PostView,Importance_Rater4_PostView)), na.rm = TRUE)
  importance_cmiyc$mean_importance_online <- rowMeans(subset(importance_cmiyc, select = c(Importance_Rater1_Online,Importance_Rater2_Online,Importance_Rater3_Online,Importance_Rater4_Online)), na.rm = TRUE)
  #replace nan with na
  importance_cmiyc$mean_importance_postview[is.nan(importance_cmiyc$mean_importance_postview)] <- NA
  importance_cmiyc$mean_importance_online[is.nan(importance_cmiyc$mean_importance_online)] <- NA
  ...
  #loop through TRs to find corresponding offsets in the interpolated timecourse
  for (i in 1:length(common_time$s)){
    common_time$position_match[i] <- if(is.finite(min(importance_cmiyc$SEG.216.End.S[importance_cmiyc$SEG.216.End.S >= common_time$s[i]]))==FALSE){NA}else{min(importance_cmiyc$SEG.216.End.S[importance_cmiyc$SEG.216.End.S >= common_time$s[i]])} #find closest offset in use.data that is greater than or equal to common_time$s[i]
    common_time$SEG216_importance_postview[i] <- as.numeric(if(is.finite(importance_cmiyc[common_time$position_match[i]==importance_cmiyc$SEG.216.End.S,]$mean_importance_postview)==FALSE){NA}else{importance_cmiyc[common_time$position_match[i]==importance_cmiyc$SEG.216.End.S,]$mean_importance_postview}) #find corresponding mean importance rating (postview)
    common_time$SEG216_importance_online[i] <- as.numeric(if(is.finite(importance_cmiyc[common_time$position_match[i]==importance_cmiyc$SEG.216.End.S,]$mean_importance_online)==FALSE){NA}else{importance_cmiyc[common_time$position_match[i]==importance_cmiyc$SEG.216.End.S,]$mean_importance_online}) #find corresponding mean importance rating (online)
  }
...
##############################################################
  #load clustering
  clustering_cmiyc_ff <- read.csv("~/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/cmiyc_long/cmiyc_long_human_rater_summary_EMH.csv")
  names(clustering_cmiyc_ff)
  ...
  #loop through TRs to find corresponding offsets in the interpolated timecourse
  for (i in 1:length(common_time$s)){
    common_time$position_match[i] <- if(is.finite(min(clustering_cmiyc_ff$offset[clustering_cmiyc_ff$offset >= common_time$s[i]]))==FALSE){NA}else{min(clustering_cmiyc_ff$offset[clustering_cmiyc_ff$offset >= common_time$s[i]])} #find closest offset in use.data that is greater than or equal to common_time$s[i]
    common_time$cluster_count[i] <- as.numeric(if(is.finite(clustering_cmiyc_ff[common_time$position_match[i]==clustering_cmiyc_ff$offset,]$count.cluster)==FALSE){NA}else{clustering_cmiyc_ff[common_time$position_match[i]==clustering_cmiyc_ff$offset,]$count.cluster}) #find corresponding number of clusters
    common_time$cluster_mean[i] <- as.numeric(if(is.finite(clustering_cmiyc_ff[common_time$position_match[i]==clustering_cmiyc_ff$offset,]$percent.mean)==FALSE){NA}else{clustering_cmiyc_ff[common_time$position_match[i]==clustering_cmiyc_ff$offset,]$percent.mean}) #find corresponding mean cluster size
    common_time$cluster_max[i] <- as.numeric(if(is.finite(clustering_cmiyc_ff[common_time$position_match[i]==clustering_cmiyc_ff$offset,]$percent.max)==FALSE){NA}else{clustering_cmiyc_ff[common_time$position_match[i]==clustering_cmiyc_ff$offset,]$percent.max}) #find corresponding max cluster size
  }
  
  #check warnings. Note: should be a ton because importance ratings were only assessed in filmfest, so no importance ratings for second half of CMIYC long
  #warnings()
  table(is.na(subset(common_time, select=c(cluster_count,cluster_mean,cluster_max))))
...
########################################################
  ...
  #load PMC data
  mean_pmc_cmiyc_ff <- as.data.frame(tc_list[which(stri_sub(stri_sub(temp,12),1,-16)=="YEO400_bilateral_PMC")]) #looks for specified ROI label in tc_list
  pisc_pmc_cmiyc_ff <- as.data.frame(pisc_list[which(stri_sub(stri_sub(temp,12),1,-16)=="YEO400_bilateral_PMC")]) #looks for specified ROI label in pisc_list
  #merge into one file
  pmc_cmiyc_ff <- merge(mean_pmc_cmiyc_ff, pisc_pmc_cmiyc_ff, by = 's')
  #delete previous files
  rm(mean_pmc_cmiyc_ff,pisc_pmc_cmiyc_ff)
  ...
  #loop through TRs to find corresponding offsets in the interpolated timecourse
  for (i in 1:length(common_time$s)){
    common_time$position_match[i] <- if(is.finite(min(pmc_cmiyc_ff$s[pmc_cmiyc_ff$s >= common_time$s[i]]))==FALSE){NA}else{min(pmc_cmiyc_ff$s[pmc_cmiyc_ff$s >= common_time$s[i]])} #find closest offset in use.data that is greater than or equal to common_time$s[i]
    common_time$pmc_mean[i] <- as.numeric(if(is.finite(pmc_cmiyc_ff[common_time$position_match[i]==pmc_cmiyc_ff$s,]$group.mean)==FALSE){NA}else{pmc_cmiyc_ff[common_time$position_match[i]==pmc_cmiyc_ff$s,]$group.mean}) #find corresponding number of clusters
    common_time$pmc_pisc[i] <- as.numeric(if(is.finite(pmc_cmiyc_ff[common_time$position_match[i]==pmc_cmiyc_ff$s,]$group.pISC)==FALSE){NA}else{pmc_cmiyc_ff[common_time$position_match[i]==pmc_cmiyc_ff$s,]$group.pISC}) #find corresponding mean cluster size
  }
...
####################################
  ...
  boundaries_target <- subset(boundaries_ff, SEG.12.title=="2. Catch Me If You Can") #specify where the data live in the dataframe
  #reset times to when the target movie began
  boundaries_target$True.Start.Time.S <- boundaries_target$Start.Time.S. - movie_start_time
  boundaries_target$True.End.Time.S <- boundaries_target$End.Time.S. - movie_start_time
  ...
  common.pos <- as.data.frame(which(common_time$SEG216_end!=0,arr.ind = T))
    ...
    diff.pos <- difference_inner_join(common.pos,boundary.pos,max_dist=5) #save output of fuzzy match function
...
#################################
  ...
  #sanity check: number of rows in full.pre and full.post should equal number of boundaries
  length(na.omit(unique(common_time$SEG1000_number)))==nrow(full.pre)
  length(na.omit(unique(common_time$SEG1000_number)))==nrow(full.post)
  ...
  ggplot(data=tr, aes(x=s, y=similarity)) +
    ...
    theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
    xlab("time (s)") + ylab("cosine similarity") + ylim(.2, .7)
  ggsave(paste0(figure_path,"cmiyc_ff","_use_upsampled_timecourse+boundaries.tiff"), dpi = 300, scale = 2:1)
  ...
  ggplot(data=common_time, aes(x=s, y=orig_use)) +
    ...
    theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
    xlab("time (s)") + ylab("cosine similarity") + ylim(.2, .7)
  ggsave(paste0(figure_path,"cmiyc_ff","_use_drag_timecourse+boundaries.tiff"), dpi = 300, scale = 2:1)
  ...
  ggplot(data=use.data, aes(x=offset, y=similarity)) +
    ...
    theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
    xlab("time (s)") + ylab("cosine similarity") + ylim(.2, .7) 
  ggsave(paste0(figure_path,"cmiyc_ff","_use_orig_timecourse+boundaries.tiff"), dpi = 300, scale = 2:1)
  ...
  ggplot(data=use.data, aes(x=offset, y=similarity)) +
    ...
    theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
    xlab("time (s)") + ylab("cosine similarity") + ylim(.2, .7) 
  ggsave(paste0(figure_path,"cmiyc","_use_orig_timecourse+SEG216boundaries.tiff"), dpi = 300, scale = 2:1)
  ...
  ggsave(paste0(figure_path,"cmiyc","_zuse_orig_timecourse+SEG216boundaries.tiff"), dpi = 300, scale = 2:1)
  ...
  ff_recall <- read.csv("~/Dropbox/Academic/Projects/JHU/FilmFest/behaviour/Hongmi_behav_data/Filmfest_recall_probability.csv", header = TRUE, comment.char="#")
    ...
    #NOTE: SEG.216 labels in CMIYC extended extends beyondd SEG.216 for CMIYC in filmfest. Recall participants only saw the FilmFest version, so only use relevant labels
    #load the full filmfest annotations
    ff_full_annot <- read.csv("~/Dropbox/Academic/Projects/JHU/FilmFest/behaviour/FilmFest_Annotation_JL_Labels(1625).csv", header = TRUE, comment.char="#")
    #take CMIYC only
    cmiyc_ff_annot <- subset(ff_full_annot, SEG.12.title=="2. Catch Me If You Can")
    max(na.omit(cmiyc_ff_annot$SEG.216.number)) #get max scene label for the filmfest version of CMIYC
    
    #subset common_event by the maximum number of CMIYC filmfest scenes
    common_event <- subset(common_event, SEG216_number<(max(na.omit(cmiyc_ff_annot$SEG.216.number))+1))

  #select target movie data only
  ff_recall <- subset(ff_recall, movie_label=="2. Catch Me If You Can")
  ...
  common_event$ff_importance_postview <- with(importance_cmiyc, mean_importance_postview[match(common_event$SEG216_number, scene.number)])
  common_event$ff_importance_online <- with(importance_cmiyc, mean_importance_online[match(common_event$SEG216_number, scene.number)])
  ...
  #quick correlation
  cor.test(common_event$consensus_up, common_event$ff_recall_probability)  
