...
setwd("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/keithreynolds/")

#create additional directories
prediction_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/keithreynolds/predictions/"
dir.create(prediction_path, showWarnings = FALSE)
figure_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/keithreynolds/figures/"
dir.create(figure_path, showWarnings = FALSE)
cosmat_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/keithreynolds/figures/cosmat/"
dir.create(cosmat_path, showWarnings = FALSE)
hist_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/keithreynolds/figures/hist/"
dir.create(hist_path, showWarnings = FALSE)
clean_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/keithreynolds/predictions/clean/"
dir.create(clean_path, showWarnings = FALSE)
raw_path <- "D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/keithreynolds/predictions/raw/"
dir.create(raw_path, showWarnings = FALSE)

##load data
#contains trial data & keywords
trial.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/keithreynolds/trialdata.csv", header = FALSE, comment.char="#")
colnames(trial.data) <- c("ID","Trial","Timestamp","Datastring")
#contains post-task survey data
question.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/keithreynolds/questiondata.csv", header = FALSE, comment.char="#")
colnames(question.data) <- c("ID","Question","Response")
#contains window events (e.g., focus on/off, window resize, etc)
event.data <- read.csv("D:/PNB/Work/College/Research/Chen_Lab/Prediction Consensus Data/New_2021/keithreynolds/eventdata.csv", header = FALSE, comment.char="#")
...
drop.loops <- c("debugRl5wZ:debugBEuIb",
...
spoiler.data$dataset <- "keithreynolds"
...
write.csv(spoiler.data, file = paste0(raw_path,"keithreynolds_spoilerdata.csv")) #save dataframe to csv
...
export_clean_prediction <- function(df,phase,counterbalance,segment){
  ...
  write.table(df, file = paste0(clean_path,"keithreynolds_",phase,"_c",counterbalance,"_seg",segment,".txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to csv
}
#define function to save all predictions in raw format
export_raw_prediction <- function(df, phase){
  write.csv(df, file = paste0(raw_path,"keithreynolds_",phase,"_full.csv")) #save full dataframe to csv
}
...
test_cAllsegTitle <- prediction.data[grep("filmfest_clip6_title_4s.mp4", prediction.data$vidpath), ]
...
#path to python script: /Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments//Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/keithreynolds/predictions
...
for(i in 1:length(use_list)){ #for all USE csv (i.e., each stop)
  ...
  if (i>2 & i<length(use_list)){ #if statement to avoid including the practice and title matrices, which are much larger than the rest
    ...
    temp_df$label <- as.character(stri_sub(stri_sub(temp[i],19),1,-5)); #add label to dataframe
  ...
  dev.print(pdf, paste0(figure_path,'cosmat/', stri_sub(stri_sub(temp[i],19),1,-5),'_cosmat.pdf'))
  ...
  dev.print(pdf, paste0(figure_path,'hist/', stri_sub(stri_sub(temp[i],19),1,-5),'_hist.pdf'))
}
...
ggplot(data = melt_cosmat, aes(x=variable, y=variable1, fill=value)) + 
  ...
  labs(title="Cosine Similarity: USE in Keith Reynolds", size = 20)
ggsave(paste0(figure_path,"keithreynolds_use_cosmat.pdf"), scale = 2)
...
colnames(df) <- stri_sub(stri_sub(temp,19),1,-5) #rename columns
...
ggplot(data=df, aes(similarity)) +
  ...
  labs(title="Histogram: USE similarity in Keith Reynolds", size = 20) +
  labs(x="Similarity", y="Count")
ggsave(paste0(figure_path,"keithreynolds_use_histogram.pdf"), scale = 2)
...
for (i in 1:length(use_list)){
  use.data$label[i] <- stri_sub(stri_sub(temp[i],19),1,-5) #add label
}
...
for (i in 1:length(use_list)){
  use.data.n$label[i] <- stri_sub(stri_sub(temp[i],19),1,-5) #add label
}
...
for (i in 1:length(use_list)){
  use.data.sd$label[i] <- stri_sub(stri_sub(temp[i],19),1,-5) #add label
}
...
ggsave(paste0(figure_path,"keithreynolds_use_timecourse.tiff"), dpi = 300, scale = 1)
ggsave(paste0(figure_path,"keithreynolds_use_timecourse.png"), dpi = 300, scale = 1)
...
ggplot(data=use.data, aes(x=offset, y=similarity)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("cosine similarity") + ylim(-.1, .8)
ggsave(paste0(figure_path,"keithreynolds_use_timecourse_labelled.png"), dpi = 300, scale = 1)

#plot confidence
#z-score confidence ratings by subject
temp.conf <- ddply(subset(prediction.data,phase=="test"), ~ID, summarise, mean = mean(confidence), sd = sd(confidence), se = sd/sqrt(length(unique(prediction.data$ID)))) #get subject means
temp.conf2 <- ddply(subset(prediction.data,phase=="test"), ~ID + offset, summarise, mean = mean(confidence)) #get means for subject per stop
temp.conf2$sub.mean <- with(temp.conf, mean[match(temp.conf2$ID, ID)]) #match subject mean
temp.conf2$sub.sd <- with(temp.conf, sd[match(temp.conf2$ID, ID)]) #match subject sd
temp.conf2$z <- (temp.conf2$mean - temp.conf2$sub.mean)/temp.conf2$sub.sd #calculate z-score
...
ggsave(paste0(figure_path,"keithreynolds_confidence_timecourse.tiff"), dpi = 300, scale = 1)
ggsave(paste0(figure_path,"keithreynolds_confidence_timecourse.png"), dpi = 300, scale = 1)
...
ggplot(data=confidence.data, aes(x=offset, y=z_mean)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("confidence (z)") + ylim(-1.1, 1.5)
ggsave(paste0(figure_path,"keithreynolds_confidence_timecourse_labelled.png"), dpi = 300, scale = 1)
...
accuracy.data <- read.csv(paste0(prediction_path,"keithreynolds_stopwise_accuracy.csv"), header = TRUE, comment.char="#")
...
ggsave(paste0(figure_path,"keithreynolds_accuracy_timecourse.png"), dpi = 300, scale = 1)
...
confidence.data$experiment <- "keithreynolds"
use.data$experiment <- "keithreynolds"
accuracy.data$experiment <- "keithreynolds"
...
ggsave(paste0(figure_path,"keithreynolds_use_conf_scatter.png"), scale = 1)
...
ggplot(confidence.data,aes(x=z_mean, y=accuracy.data$similarity))+
  ...
  scale_x_continuous(limits = c(-.8,.8)) +
  stat_cor(method = "pearson", label.x = (median((-8:8)/10)), label.y = .675) #from ggpubr -- plots r and p-value
ggsave(paste0(figure_path,"keithreynolds_accuracy_conf_scatter.png"), scale = 1)
...
ggsave(paste0(figure_path,"keithreynolds_accuracy_use_scatter.png"), scale = 1)
...
#build a vector representing each TR, in seconds, for the keithreynolds movie
tr <- as.data.frame(seq(from = 0, to = 359, by = 1.5))
...
write.table(df.orig.use, file = paste0(prediction_path,"keithreynolds_use_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.use, file = paste0(prediction_path,"keithreynolds_use_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
df.orig.conf <- subset(confidence.data, select = z_mean)
write.table(df.orig.conf, file = paste0(prediction_path,"keithreynolds_conf_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.conf, file = paste0(prediction_path,"keithreynolds_conf_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
df.orig.acc <- subset(accuracy.data, select = similarity)
write.table(df.orig.acc, file = paste0(prediction_path,"keithreynolds_acc_orig.txt"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.orig.acc, file = paste0(prediction_path,"keithreynolds_acc_orig.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv

write.csv(tr, file = paste0(prediction_path,"keithreynolds_full_interp_tr_match.csv")) #save full dataframe to csv
...
#write.table(df.use, file = paste0(prediction_path,"keithreynolds_use_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#write.table(df.use, file = paste0(prediction_path,"keithreynolds_use_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
write.table(df.use, file = paste0(prediction_path,"keithreynolds_use_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.use, file = paste0(prediction_path,"keithreynolds_use_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
...
#write.table(df.conf, file = paste0(prediction_path,"keithreynolds_conf_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#write.table(df.conf, file = paste0(prediction_path,"keithreynolds_conf_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
write.table(df.conf, file = paste0(prediction_path,"keithreynolds_conf_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.conf, file = paste0(prediction_path,"keithreynolds_conf_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
...
#write.table(df.acc, file = paste0(prediction_path,"keithreynolds_acc_tr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
#write.table(df.acc, file = paste0(prediction_path,"keithreynolds_acc_tr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
write.table(df.acc, file = paste0(prediction_path,"keithreynolds_acc_filmfesttr_match.csv"), row.names=FALSE, col.names=FALSE, sep=",", quote = FALSE) #save prediction column only, without row or column labels, to csv
write.table(df.acc, file = paste0(prediction_path,"keithreynolds_acc_filmfesttr_match.txt"), row.names=FALSE, col.names=FALSE, sep=" ", quote = FALSE) #save prediction column only, without row or column labels, to txt
...
ggsave(paste0(figure_path,"keithreynolds_use_upsampled_timecourse.png"), dpi = 300, scale = 1)
...
ggsave(paste0(figure_path,"keithreynolds_confidence_upsampled_timecourse.png"), dpi = 300, scale = 1)
...
ggplot(data=tr, aes(x=s, y=accuracy)) +
  ...
  theme(text=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(colour="black", size = 1)) +
  xlab("time (s)") + ylab("accuracy") + ylim(.2, .7)
ggsave(paste0(figure_path,"keithreynolds_accuracy_upsampled_timecourse.png"), dpi = 300, scale = 1)
...
setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/keithreynolds/roi_tc/tc")
temp = list.files(pattern="*.txt")
tc_list = lapply(temp, function(x) read.delim(x, header = FALSE, sep = ","))
#load group averaged pISC ROI timecourses
setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/keithreynolds/roi_tc/pisc")
...
for (i in 1:length(tc_list)){
  ...
  #set path
  setwd("/Users/bbellan1/Dropbox/Academic/Projects/JHU/online_experiments/prediction_probe/analysis_sandbox/keithreynolds/roi_tc/")
  ...
  ggsave(paste0("keithreynolds_",stri_sub(stri_sub(temp[i],12),1,-16),"_timecourses.png"), scale = 2)
...
common_time <- as.data.frame(seq(from = 0, to = 359, by = .5))
...
#isolate Keith Reynolds only and start timings at actual movie onsets
table(annot_movie$SEG.12.title)
annot_movie_clean <- subset(annot_movie, SEG.12.title %in% c("6. Keith Reynolds"))
...
#clean up
importance_target_movie <- head(tail(importance_ff,136),26) #manually specify where the data live in the dataframe
...
boundaries_target <- subset(boundaries_ff, SEG.12.title=="6. Keith Reynolds") #specify where the data live in the dataframe
...
#export annotations in USE-compatible format
  ...
  ff_recall <- subset(ff_recall, movie_label=="6. Keith Reynolds")
